...
CAJA_CUPON_IMAGEN = {
    10: {"url": "https://www.mibookeo.es/descuento10.png", "left": 43.05, "top": 81.17, "width": 50.10, "height": 15.33, "font_size": 16},
    15: {"url": "https://www.mibookeo.es/descuento15.png", "left": 24.95, "top": 73.00, "width": 67.81, "height": 16.67, "font_size": 20},
    ...
    return f"""
      <div style="position:relative;width:100%;max-width:520px;margin:0 auto 6px;">
        ...
        <div style="position:absolute;left:{datos['left']}%;top:{datos['top']}%;width:{datos['width']}%;height:{datos['height']}%;text-align:center;">
          <span style="font-family:Georgia,serif;font-weight:800;font-size:{datos['font_size']}px;color:{color};letter-spacing:0.03em;white-space:nowrap;">{codigo}</span>
        </div>
      </div>
...
def enviar_correo_confirmacion_pedido(correo, numero_pedido, libros, total_texto, envio, pdf_adjuntos=None):
    ...
    cuerpo = f"""
      ...
      {_parrafo(f"Guarda este correo: el PDF adjunto es tu copia de seguridad. Si más adelante quieres repetir el libro o necesitas que te lo volvamos a enviar, escríbenos a <a href='mailto:info@mibookeo.es' style='color:{VERDE};text-decoration:none;'>info@mibookeo.es</a>.")}
    """
...
def enviar_correo_valoracion_cupones(correo, numero_pedido, url_valoracion, codigo_fidelidad, codigo_recomendacion, titulo_libro=None):
    ...
    cuerpo = f"""
      ...
      {_parrafo(f"Por cierto: guarda el correo de confirmación de tu pedido, ahí tienes el PDF de tu libro. Si lo pierdes y quieres una copia o repetir el pedido, escríbenos a <a href='mailto:info@mibookeo.es' style='color:{VERDE};text-decoration:none;'>info@mibookeo.es</a>.")}
    """
