...
def generar_propuestas_task(self, datos: dict):
    ...
    pedido_id = datos.get("pedido_id", "sin_id")
    # OJO: el Worker corre en un contenedor DISTINTO al servicio web y no
    # comparte disco con él - work_dir se crea aquí, en el propio Worker,
    # no se recibe de main.py (antes se recibía una ruta del web y se
    # intentaba leer aquí, pero ese directorio no existía en este
    # contenedor: "No such file or directory" en cuanto se intentaba
    # tocar cualquier archivo). Las fotos/vídeos llegan como CLAVES de R2
    # (main.py ya las subió él mismo antes de lanzar esta tarea) y aquí
    # se descargan a este work_dir propio antes de trabajar con ellas.
    ...
    try:
        fotos_meta = datos["fotos_meta"]
        videos_meta = datos["videos_meta"]
        titulo = datos["titulo"]
        ...
        def _descargar_de_r2(item):
            url = generar_url_descarga(item["clave_r2"])
            resp = requests.get(url, timeout=60)
            resp.raise_for_status()
            dest = work_dir / item["nombre"]
        ...
        with ThreadPoolExecutor(max_workers=16) as pool:
            futuros = {pool.submit(_descargar_de_r2, item): item["nombre"] for item in fotos_meta}
            for fut in as_completed(futuros):
                nombre = futuros[fut]
                try:
                    fotos_rutas.append(fut.result())
                except Exception as e:
                    print(f"[DEBUG] ERROR descargando foto {nombre} de R2: {e}")
                    errores_descarga.append(nombre)

        if errores_descarga:
            raise RuntimeError(
                f"Error descargando {len(errores_descarga)} foto(s) del almacenamiento temporal: "
                f"{', '.join(errores_descarga[:5])}"
            )
        print(f"[DEBUG] {len(fotos_rutas)} fotos descargadas de R2 al disco del Worker (en paralelo)")

        self.update_state(state="PROGRESS", meta={"estado": "subiendo_videos", "pedido_id": pedido_id})

        videos_rutas = []
        for item in videos_meta:
            try:
                videos_rutas.append(_descargar_de_r2(item))
            except Exception as e:
                raise RuntimeError(f"Error descargando vídeos del almacenamiento temporal: {e}")
        print(f"[DEBUG] {len(videos_rutas)} vídeos descargados de R2 al disco del Worker")

        # Las claves de R2 de fotos/vídeos ya las tenía main.py (fue quien
        # subió los originales) - se reutilizan tal cual para el resultado
        # final, no hace falta volver a subir nada aquí.
        fotos_r2_claves = {item["nombre"]: item["clave_r2"] for item in fotos_meta}
        videos_r2_claves = {item["nombre"]: item["clave_r2"] for item in videos_meta}
