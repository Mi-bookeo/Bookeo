...
app.conf.update(
    ...
    # Tarea programada: revisa cada día a las 10:00 (hora de Madrid) los
    # pedidos sin pagar de hace 7 días y los pagados de hace 7 días -
    # OJO: esto NO se ejecuta solo por estar aquí. Hace falta un proceso
    # aparte en Railway corriendo "celery -A celery_worker beat" además
    # del worker normal ("celery -A celery_worker worker") - son dos
    # procesos distintos, el worker ejecuta tareas, el beat es el
    # "reloj" que las dispara a su hora.
    beat_schedule={
        "revisar-pedidos-cada-dia": {
            "task": "revisar_pedidos_periodicos",
            "schedule": crontab(hour=10, minute=0),
        },
        "resumen-facturacion-mensual": {
            "task": "enviar_resumen_facturacion_mensual",
            # Día 1 de cada mes a las 09:00 - resume el mes ANTERIOR
            # completo (si hoy es 1 de octubre, resume septiembre).
            "schedule": crontab(hour=9, minute=0, day_of_month=1),
        },
        "resumen-facturacion-trimestral": {
            "task": "enviar_resumen_facturacion_trimestral",
            # Día 1 de enero/abril/julio/octubre a las 09:30 - resume
            # los 3 meses anteriores completos.
            "schedule": crontab(hour=9, minute=30, day_of_month=1, month_of_year="1,4,7,10"),
        },
    },
)
...
def calcular_paginas_libro(self, datos: dict):
    ...
    try:
        ...
        videos_completos = [
            {"nombre": nombre, "url": url, "thumb": thumb_urls.get(nombre)}
            for nombre, url in datos.get("qr_urls", {}).items()
        ]
        ...
        try:
            ...
            pass
        # OJO: NO se llama a self.update_state(state="FAILURE", ...) a
        # mano - Celery espera que el resultado de un estado FAILURE
        # tenga una forma concreta (exc_type/exc_module/exc_message) para
        # poder reconstruir la excepción después; un meta cualquiera como
        # {"error": ...} no vale, y RESULTADO_TAREA.result (o /estado-
        # tarea, que lo consulta) rompe con "ValueError: Exception
        # information must include the exception type" al intentar leerlo
        # más tarde. Basta con relanzar la excepción tal cual (raise) -
        # Celery la captura solo y la guarda bien formada de por sí.
        raise
...
def generar_libro(self, datos: dict):
    ...
    try:
        ...
        # Ver comentario en calcular_paginas_libro sobre por qué no se
        # llama a self.update_state(state="FAILURE", ...) a mano.
        raise


@app.task(name="revisar_pedidos_periodicos")
def revisar_pedidos_periodicos():
    """
    Se dispara sola cada día a las 10:00 (ver beat_schedule arriba).
    Dos revisiones independientes, cada pedido se avisa una sola vez
    (por eso los "marcar_..._enviado" al final de cada uno - si algo
    falla a mitad, ese pedido en concreto se reintentará mañana, pero
    los que ya se avisaron con éxito no se vuelven a avisar).
    """
    from supabase_client import (
        obtener_pedidos_sin_pagar_hace_dias, marcar_aviso_pago_enviado,
        obtener_pedidos_para_valorar_hace_dias, marcar_valoracion_enviada,
        obtener_cupones_cliente,
    )
    from resend_client import enviar_correo_aviso_pago_pendiente, enviar_correo_valoracion_cupones

    # 1) Pedidos sin pagar desde hace 7 días - aviso de borrado.
    for pedido in obtener_pedidos_sin_pagar_hace_dias(7):
        numero_pedido = pedido.get("id")
        try:
            if pedido.get("correo"):
                enviar_correo_aviso_pago_pendiente(
                    correo=pedido["correo"],
                    numero_pedido=numero_pedido,
                    url_reintentar=f"{URL_SITIO}/pago.html",
                )
            marcar_aviso_pago_enviado(numero_pedido)
        except Exception as e:
            print(f"[DEBUG] ERROR avisando pago pendiente del pedido {numero_pedido}: {e}")

    # 2) Pedidos pagados hace 7 días - valoración + cupones. Los códigos
    # ya existen desde que se confirmó el pago (ver crear_cupones_cliente
    # en el webhook de Stripe) - aquí solo se LEEN, nunca se generan de
    # nuevo.
    for pedido in obtener_pedidos_para_valorar_hace_dias(7):
        numero_pedido = pedido.get("id")
        try:
            if pedido.get("correo"):
                cupon = obtener_cupones_cliente(pedido.get("cliente_id"))
                if cupon and cupon.get("cupon") and cupon.get("cupon_amigo"):
                    enviar_correo_valoracion_cupones(
                        correo=pedido["correo"],
                        numero_pedido=numero_pedido,
                        url_valoracion=f"{URL_SITIO}/valorar.html?correo={pedido['correo']}&pedido={numero_pedido}",
                        codigo_fidelidad=cupon["cupon"],
                        codigo_recomendacion=cupon["cupon_amigo"],
                        titulo_libro=pedido.get("titulo_libro"),
                    )
                else:
                    print(f"[DEBUG] Pedido {numero_pedido} pagado hace 7 días pero sin cupones que mandar (¿ya usados?)")
            marcar_valoracion_enviada(numero_pedido)
        except Exception as e:
            print(f"[DEBUG] ERROR enviando valoración del pedido {numero_pedido}: {e}")


def _primer_dia_de_mes(fecha):
    return fecha.replace(day=1)


def _mes_anterior(fecha):
    """Devuelve el primer día del mes anterior a 'fecha'."""
    primer_dia_actual = _primer_dia_de_mes(fecha)
    return (primer_dia_actual - datetime.timedelta(days=1)).replace(day=1)


@app.task(name="enviar_resumen_facturacion_mensual")
def enviar_resumen_facturacion_mensual():
    """
    Se dispara sola el día 1 de cada mes (ver beat_schedule). Resume el
    mes ANTERIOR completo - si hoy es 1 de octubre, manda el resumen de
    todo septiembre.
    """
    from supabase_client import obtener_pedidos_pagados_entre
    from resend_client import enviar_correo_resumen_facturacion

    hoy = datetime.date.today()
    inicio_mes_anterior = _mes_anterior(hoy)
    inicio_este_mes = _primer_dia_de_mes(hoy)

    pedidos = obtener_pedidos_pagados_entre(inicio_mes_anterior, inicio_este_mes)
    total = sum(p.get("precio") or 0 for p in pedidos)

    meses_es = ["enero", "febrero", "marzo", "abril", "mayo", "junio",
                "julio", "agosto", "septiembre", "octubre", "noviembre", "diciembre"]
    titulo = f"{meses_es[inicio_mes_anterior.month - 1].capitalize()} {inicio_mes_anterior.year}"

    try:
        enviar_correo_resumen_facturacion(titulo, pedidos, total)
    except Exception as e:
        print(f"[DEBUG] ERROR enviando el resumen de facturación mensual ({titulo}): {e}")


@app.task(name="enviar_resumen_facturacion_trimestral")
def enviar_resumen_facturacion_trimestral():
    """
    Se dispara sola el día 1 de enero/abril/julio/octubre (ver
    beat_schedule). Resume los 3 meses anteriores completos - si hoy es
    1 de octubre, manda julio+agosto+septiembre juntos.
    """
    from supabase_client import obtener_pedidos_pagados_entre
    from resend_client import enviar_correo_resumen_facturacion
    ...
    hoy = datetime.date.today()
    inicio_este_mes = _primer_dia_de_mes(hoy)
    # Retrocede 3 meses uno a uno, para no depender de ninguna librería
    # aparte solo por esta resta de fechas.
    inicio_trimestre = inicio_este_mes
    for _ in range(3):
        inicio_trimestre = _mes_anterior(inicio_trimestre)

    pedidos = obtener_pedidos_pagados_entre(inicio_trimestre, inicio_este_mes)
    total = sum(p.get("precio") or 0 for p in pedidos)

    meses_es = ["enero", "febrero", "marzo", "abril", "mayo", "junio",
                "julio", "agosto", "septiembre", "octubre", "noviembre", "diciembre"]
    fin_trimestre = _mes_anterior(inicio_este_mes)
    titulo = f"{meses_es[inicio_trimestre.month - 1].capitalize()}-{meses_es[fin_trimestre.month - 1].capitalize()} {inicio_trimestre.year}"

    try:
        enviar_correo_resumen_facturacion(titulo, pedidos, total)
    except Exception as e:
        print(f"[DEBUG] ERROR enviando el resumen de facturación trimestral ({titulo}): {e}")