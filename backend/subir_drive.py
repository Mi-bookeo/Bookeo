...
def obtener_o_crear_subcarpeta_pedido(service, carpeta_principal_id, pedido_id, nombre_album, cliente_id=None):
    ...
    """
    # Un pedido nuevo (la inmensa mayoria de los casos, ya que cada pedido
    # es unico) todavia no tiene NINGUNA fila en la tabla 'pedidos'. Segun
    # como este montada esa consulta en supabase_client.py (con .single()),
    # Supabase no devuelve "nada" en ese caso - lanza un error (PGRST116:
    # "The result contains 0 rows") en vez de None. Sin capturarlo aqui,
    # tira toda la peticion con un 500 en CADA pedido nuevo. Se trata igual
    # que "todavia no tiene subcarpeta" - que es exactamente lo que es.
    try:
        subcarpeta_id = obtener_subcarpeta_pedido(pedido_id)
    except Exception as e:
        log(f"Pedido {pedido_id} sin subcarpeta previa (normal en pedido nuevo): {e}", "i")
        subcarpeta_id = None
    ...
    # cliente_id se pasa aquí porque, si todavía no existe ninguna fila de
    # 'pedidos' para este pedido_id (el vídeo se ha subido durante la
    # edición, antes de generar el PDF final, que es cuando normalmente se
    # crea esa fila), guardar_subcarpeta_pedido tiene que INSERTAR una fila
    # nueva - y 'cliente_id' es obligatorio (NOT NULL) en esa tabla. Sin
    # esto, el insert fallaba y el vídeo se perdía.
    guardar_subcarpeta_pedido(pedido_id, subcarpeta_id, cliente_id=cliente_id)
