...
def crear_pedido_drive_estado(pedido_id: str):
    ...
    este pedido tiene Drive conectado. El editor la llama justo ANTES de
    dejar elegir un vídeo - así, si no está conectado, se avisa al
    instante sin hacer subir el archivo entero primero para enterarse
    ...
    if not cliente_id:
        return {"conectado": False}
    datos_drive = obtener_cliente_drive(cliente_id)
    return {"conectado": bool(datos_drive and datos_drive.get("google_refresh_token"))}
