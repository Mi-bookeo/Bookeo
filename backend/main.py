...
async def crear_pedido_confirmar(
    ...
    # AVISO TEMPORAL para diagnosticar lo de "portada ampliada" - se puede
    # quitar en cuanto quede claro qué está pasando. Vuelca en los logs de
    # Railway justo el dato que hace falta ver: si foto_ref_w/h llegan con
    # un número o no llegan en absoluto.
    if editor and editor.get("foto"):
        f_dbg = editor["foto"]
        print(f"[DEBUG PORTADA] foto_ref_w={f_dbg.get('foto_ref_w')!r} "
              f"foto_ref_h={f_dbg.get('foto_ref_h')!r} "
              f"cropX={f_dbg.get('cropX')!r} cropY={f_dbg.get('cropY')!r} "
              f"width={f_dbg.get('width')!r} height={f_dbg.get('height')!r} "
              f"scaleX={f_dbg.get('scaleX')!r} scaleY={f_dbg.get('scaleY')!r} "
              f"portada_foto={portada_foto!r}")
