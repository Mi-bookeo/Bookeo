...
from crear_libro_railway import generar_propuestas_portada, leer_fecha
...
async def crear_pedido_propuestas(
    ...
    print(f"[DEBUG] Petición recibida: pedido_id={pedido_id}, cliente_id={cliente_id}, sin_capitulos={sin_capitulos}")
    ...
    try:
        ...
        for foto in fotos:
            ...
            try:
                clave_r2 = subir_a_r2(str(dest), pedido_id, foto.filename)
                fotos_r2_claves[foto.filename] = clave_r2
            except Exception as e:
                print(f"[DEBUG] ERROR subiendo foto {foto.filename} a R2: {e}")
                raise HTTPException(
                    status_code=500,
                    detail=f"Error subiendo fotos al almacenamiento temporal: {e}"
                )
        print(f"[DEBUG] {len(fotos_rutas)} fotos guardadas en disco y subidas a R2")
...
async def crear_pedido_subir_video(
    ...
    try:
        ...
        print(traceback.format_exc())
        raise HTTPException(status_code=500, detail=f"Error subiendo el vídeo a Drive: {e}")
...
def estado_tarea(tarea_id: str):
    ...
    if resultado.state == "PENDING":
        ...
        if "pdf_url" in r:
            return {"estado": "listo", "pdf_url": r.get("pdf_url")}
        else:
