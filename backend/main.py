...
async def crear_pedido_propuestas(
    ...
    el servidor terminaba bien pero el navegador ya no estaba ahí para
    recibir la respuesta, y se quedaba "pensando" para siempre aunque el
    log dijera "propuestas generadas correctamente". Ahora esta petición
    solo guarda los archivos subidos en disco (rápido, sin red de por
    medio) y lanza una tarea de Celery con todo el trabajo pesado,
    devolviendo un tarea_id al momento - el navegador sondea
    /estado-tarea/{tarea_id} cada 3 segundos, igual que ya hace para el
    PDF final. Un corte de conexión puntual solo pierde ese sondeo
    concreto, no el resultado entero.
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error guardando los archivos subidos: {e}")

    print(f"[DEBUG] {len(fotos_rutas)} fotos y {len(videos_rutas)} vídeos guardados en disco - lanzando tarea de propuestas")
