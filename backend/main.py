...
def _crear_sesion_stripe(numero_pedido, libros, envio, total):
    """
...
async def crear_pedido_iniciar_pago(datos: ConfirmarPagoSimulado):
    ...
    sesion = _crear_sesion_stripe(numero_pedido, datos.libros, datos.envio, datos.total)
...
async def webhook_stripe(request: Request):
    ...
    if tipo == "checkout.session.completed":
        ...
        # Aviso de pago fallido - el correo (enviar_correo_pago_fallido)
        # ya está escrito en resend_client.py, listo para conectarlo aquí
        # en cuanto se decida la URL a la que debe volver el cliente.
        print(f"[DEBUG] Pago fallido/expirado: {evento.get('data', {}).get('object', {}).get('id')}")
