...
def auth_google_iniciar(pedido_id: str = None):
    ...
    # editor (ver abrirModalDriveDesconectado() en editor.html) - se
    # manda a Google como 'state' y Google nos lo devuelve tal cual en
    # el callback, así sabemos a dónde volver.
    url = generar_url_autorizacion(state=pedido_id)
...
def auth_google_callback(code: str = None, error: str = None, state: str = None):
    ...
    for intento in range(2):
        try:
            ...
            # Si venimos de una reconexión pedida desde dentro del editor
            # (state = pedido_id), volvemos ahí en vez de al creador - el
            # editor ya se encargó de guardar el estado en sessionStorage
            # antes de salir, así que al cargar lo recupera tal cual.
            if state:
