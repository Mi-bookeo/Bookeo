...
def ver_pdf(tarea_id: str):
    """
    Sirve el PDF final SOLO para visionado online (lo consume el visor de
    pdf.js del front, que lo pinta en un <canvas>) - nunca se manda al
    navegador la URL real de R2, así que no hay ningún enlace que el
    cliente pueda copiar/abrir para descargarse directamente el archivo
    listo para imprimir. El PDF de verdad solo se entrega tras el pago,
    por otro canal (email/pedido), no desde aquí.
    """
    from celery_worker import app as celery_app
    resultado = celery_app.AsyncResult(tarea_id)
    if resultado.state != "SUCCESS":
