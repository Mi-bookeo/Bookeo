...
def guardar_subcarpeta_pedido(pedido_id, subcarpeta_drive_id):
    ...
    del pedido en algún caso límite (por ejemplo si Drive se conecta antes
    de que el PDF haya empezado a generarse) - así nunca falla en
    silencio como pasaba antes.
    """
    supabase.table("pedidos").upsert(
        {"id": pedido_id, "subcarpeta_drive_id": subcarpeta_drive_id},
        on_conflict="id",
    ).execute()
