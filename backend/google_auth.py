...
def generar_url_autorizacion(state=None):
    """
    Genera la URL a la que se redirige al cliente para que autorice
    su cuenta de Google.

    'state' es opcional: se usa SOLO cuando el login se pide desde dentro
    del editor (Drive se desconectó a mitad de sesión y hace falta volver
    a conectar sin perder el libro que ya se estaba editando) - lleva el
    pedido_id para que el callback sepa que tiene que devolver al cliente
    a editor.html en vez de a creador.html, que es el destino de siempre
    cuando no hay 'state' (primera conexión, desde el creador).
    """
    flow = Flow.from_client_config(CLIENT_CONFIG, scopes=SCOPES, redirect_uri=REDIRECT_URI)
    parametros = dict(access_type="offline", prompt="consent", include_granted_scopes="true")
    if state:
        parametros["state"] = state
    url, _ = flow.authorization_url(**parametros)
    return url
