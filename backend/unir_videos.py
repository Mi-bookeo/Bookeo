...
def obtener_bitrate_kbps(ruta_video):
    """Obtiene el bitrate total (vídeo+audio) del archivo en kb/s,
    leyendo la línea 'bitrate: XXXX kb/s' que ffmpeg escribe en stderr."""
    try:
        result = subprocess.run(
            [FFMPEG_BIN, "-i", ruta_video],
            capture_output=True, text=True
        )
        m = re.search(r"bitrate:\s*(\d+)\s*kb/s", result.stderr)
        if not m:
            return None
...
def reducir_video(ruta_entrada, ruta_salida=None, carpeta_temp=None):
    ...
    Enfoque por PESO, no por calidad fija:
      1. Se calcula el bitrate que "debería" tener el vídeo segun su duración
         (target_kbps) para no pesar mas de lo razonable.
      2. Se mide el bitrate REAL del vídeo que ha subido el cliente.
      3. Si el vídeo YA viene comprimido por debajo de ese objetivo (típico de
         vídeos reenviados por WhatsApp, Instagram, etc., que ya comprimen
         mucho), NO se re-codifica: se copia tal cual. Re-codificar algo que
         ya viene muy comprimido con una calidad/resolución fija puede hacer
         que el archivo CREZCA en vez de reducirse (esto es justo lo que
         pasó: un vídeo de WhatsApp de 30MB subió a 50MB).
      4. Si el vídeo viene "en bruto" de cámara (bitrate alto, sin comprimir
         apenas), se re-codifica limitando el bitrate al objetivo, para que
         el resultado nunca supere el peso calculado.

    Devuelve la ruta del vídeo final (reducido o copiado tal cual).
    """
    if not verificar_ffmpeg():
        raise RuntimeError("FFmpeg no disponible en el servidor")
    ...
    if not duracion:
        duracion = 60  # fallback prudente si no se puede leer la duración
    ...
    peso_entrada_mb = os.path.getsize(ruta_entrada) / (1024 * 1024)

    # Objetivo de calidad/peso segun duracion (esto NO es un tope de PDF,
    # es solo la calidad que buscamos si hay que re-codificar)
    if duracion < SEGUNDOS_CORTE_CALIDAD:
        dims = (1920, 1080)
        video_kbps_objetivo = 2500
        preset = "medium"
    else:
        dims = (1280, 720)
        video_kbps_objetivo = 1200
        preset = "fast"

    audio_kbps = 128
    target_kbps = video_kbps_objetivo + audio_kbps

    bitrate_entrada = obtener_bitrate_kbps(ruta_entrada)

    # Si el vídeo YA pesa menos (o casi lo mismo) que nuestro objetivo,
    # no lo tocamos: copiarlo es mas rapido y GARANTIZA que no crezca.
    if bitrate_entrada and bitrate_entrada <= target_kbps * 1.15:
        log(f"Reductor: el vídeo ya viene comprimido ({bitrate_entrada}kb/s, "
            f"objetivo {target_kbps}kb/s) - se copia tal cual, sin re-codificar", "✅")
        shutil.copy2(ruta_entrada, ruta_salida)
        peso_salida_mb = os.path.getsize(ruta_salida) / (1024 * 1024)
        log(f"Reductor: {peso_entrada_mb:.1f}MB -> {peso_salida_mb:.1f}MB (sin cambios, ya era eficiente)", "✅")
        return ruta_salida

    # El vídeo viene "en bruto" (bitrate alto): re-codificamos limitando
    # el bitrate al objetivo, para que el resultado nunca lo supere.
    ...
    cmd = [
        ...
        "-c:v", "libx264",
        "-threads", "2",
        "-b:v", f"{video_kbps_objetivo}k",
        "-maxrate", f"{int(video_kbps_objetivo * 1.3)}k",
        "-bufsize", f"{video_kbps_objetivo * 2}k",
        "-preset", preset,
        "-c:a", "aac",
        "-b:a", f"{audio_kbps}k",
        "-ar", "44100",
    ...
    log(f"Reductor: {bitrate_entrada or '?'}kb/s -> recodificando a ~{target_kbps}kb/s "
        f"({dims[0]}x{dims[1]})", "⚙")
    ...
    # Red de seguridad final: si por lo que sea el resultado pesa MAS que el
    # original (clip muy corto y con mucho movimiento, caso raro), nos
    # quedamos con el original - nunca se entrega algo mas pesado de lo subido.
    if peso_salida_mb >= peso_entrada_mb:
        log(f"Reductor: el resultado ({peso_salida_mb:.1f}MB) no mejora el original "
            f"({peso_entrada_mb:.1f}MB) - se devuelve el original sin tocar", "⚠")
        shutil.copy2(ruta_entrada, ruta_salida)
        peso_salida_mb = os.path.getsize(ruta_salida) / (1024 * 1024)

    log(f"Reductor: {peso_entrada_mb:.1f}MB -> {peso_salida_mb:.1f}MB (misma duración: {duracion:.0f}s)", "✅")
    return ruta_salida
