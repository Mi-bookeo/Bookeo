...
def detectar_caras(ruta):
    ...
    try:
        ...
        if max(img_pil.size) > MAX_LADO:
            ...
            nuevo_tam = (int(img_pil.size[0] * ratio), int(img_pil.size[1] * ratio))
...
def dibujar_marco_pdf(c, AW, AH, marco, canvas_w):
    ...
    margen_pt = marco.get("margen", 14) * escala_pt
    color = hex_a_rgb01(marco.get("color", "#1a1a2e"))

    x0, y0 = margen_pt, margen_pt
    w, h = aw_pt - margen_pt * 2, ah_pt - margen_pt * 2
    ...
    if estilo == "simple":
        ...
        borde = max(3 * mm, aw_pt * 0.045)
        borde_abajo = borde * 2.6
        c.setFillColorRGB(1, 1, 1)
        c.rect(0, ah_pt - borde, aw_pt, borde, fill=1, stroke=0)
        c.rect(0, 0, borde, ah_pt, fill=1, stroke=0)
        c.rect(aw_pt - borde, 0, borde, ah_pt, fill=1, stroke=0)
        c.rect(0, 0, aw_pt, borde_abajo, fill=1, stroke=0)
...
def dibujar_portada_editor(c, AW, AH, ruta, titulo, subtitulo, do_wm, editor):
    ...
    if ruta and os.path.exists(ruta) and foto_info:
        try:
            ...
            ancho_nat = foto_info.get("width") or img_original.width
            alto_nat = foto_info.get("height") or img_original.height
            ...
            buf = io.BytesIO()
            img_original.save(buf, "JPEG", quality=95)
            buf.seek(0)
...
def generar_pdf(AW, AH, diseño, fotos_dict, qr_map, ruta, titulo, subtitulo, portada_elegida=None, do_wm=False, caso_reparto="A", paginas_objetivo=30):
    ...
    c.setTitle(titulo)
    c.setAuthor("Bookeo - mibookeo.es")
