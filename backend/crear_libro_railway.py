...
def generar_propuestas_portada(fotos_rutas, videos_rutas, titulo_cliente="", formato="2128", orientacion="v", packs_extra=0, sin_capitulos=False):
    ...
    # Leer la fecha (EXIF/nombre/mtime) de cada foto es I/O por archivo -
    # con cientos de fotos, hacerlo una a una antes de ni siquiera llamar a
    # Claude ya sumaba varios segundos de espera para el cliente antes de
    # ver las propuestas de portada. En paralelo con threads (es I/O, no
    # CPU, así que el GIL no lo frena) tarda una fraccion de eso.
    fotos = []
...
def _evitar_fotograma_solo(paginas, AW, AH):
    """
    Un fotograma de vídeo nunca debe ocupar una página para él solo (a
    página completa) - como mucho un cuarto de página, siempre compartiendo
    hueco con otras fotos. Si alguno acabó solo en su propia página
    (layout "1"), se fusiona con la página de contenido más cercana que
    tenga hueco libre (menos de 4 fotos), en vez de dejarlo campar solo.
    """
    LAYOUT_POR_N = {2: "2H", 3: "3", 4: "4"}
    i = 0
    while i < len(paginas):
        pg = paginas[i]
        if (pg.get("tipo") == "contenido" and pg.get("layout") == "1"
                and len(pg.get("fotos", [])) == 1
                and pg["fotos"][0].get("es_frame_video")):

            frame_foto = pg["fotos"][0]
            ...
            for j in (i - 1, i + 1):
                if 0 <= j < len(paginas):
                    candidata = paginas[j]
                    if candidata.get("tipo") == "contenido" and len(candidata.get("fotos", [])) < 4:
                        destino = candidata
                        break

            if destino is not None:
                if frame_qr_idx == 0 and frame_qr_url and destino.get("qr_idx", -1) == -1:
                ...
                # El hueco cambió de tamaño (antes página entera, ahora un
                # cuarto o menos) - hay que recalcular qué ventana de la
                # foto enseñar para el hueco nuevo, si no el recorte
                # quedaría pensado para la proporción equivocada.
                try:
...
def dibujar_pdf_desde_estructura(paginas, AW, AH, ruta, titulo, subtitulo, portada_elegida=None, do_wm=False, formato="2128"):
    ...
    for pagina in paginas:
        ...
        if tipo == "blanco":
            ...
            modo_libre = any(f.get("editor_foto") for f in fotos_cap)
            ...
            if modo_libre:
                ...
                for f in fotos_cap:
                    ...
                    if info:
                        ...
                        if not ok:
                            foto_zona(c, f["ruta"], MG, MG, AW - MG * 2, AH * 0.62 - MG)
                    else:
            ...
            if modo_libre:
                ...
                if pagina.get("qr_url"):
                    ...
                    dibujar_qr_sobre_foto(c, 0, 0, AW, AH, pagina.get("qr_url"), None, centro_custom_mm=centro_mm)
