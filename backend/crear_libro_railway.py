...
# ═══════════════════════════════════════════════════════
#  CASO B — REPARTO PROPORCIONAL POR CAPÍTULO (N > P)
# ═══════════════════════════════════════════════════════

def agrupar_por_dia(fotos_capitulo):
    """
    Agrupa las fotos de un capítulo en bloques por día (misma fecha).
    Devuelve una lista de bloques, cada uno una lista de fotos del mismo día,
...
def calcular_paginas_por_capitulo(capitulos_con_fotos, paginas_disponibles):
    """
    Calcula cuántas páginas le corresponden a cada capítulo,
    proporcionalmente a cuántas fotos tiene respecto al total.

    capitulos_con_fotos → lista de listas de fotos (una lista por capítulo)
    paginas_disponibles → P, el total de páginas objetivo para todo el libro

    Devuelve una lista de enteros (páginas asignadas a cada capítulo, en el
    mismo orden que capitulos_con_fotos). Mínimo 1 página por capítulo.
    """
  ...
  def paginas_para_capitulo_caso_b(fotos_capitulo, paginas_asignadas, qr_map, texto_titulo="", variante_inicio=0):
    ...
    fotos_capitulo    → fotos de este capítulo (ya sin las 1-2 que van
                         en la página de título, esas se tratan aparte)
    paginas_asignadas → páginas objetivo para este capítulo (puede excederse
                         si un día concreto tiene demasiadas fotos)
    qr_map            → igual que en paginas_para_grupo
    texto_titulo      → pie de página opcional para la primera página
    variante_inicio   → para alternar layouts sin repetir

    Devuelve: lista de páginas (mismo formato que paginas_para_grupo)
    """
    ...
    # Fotos por página objetivo, como media (puede no ser entero)
    ...
    for bloque in bloques:
        # Cuántas páginas le tocarían a este bloque, proporcional a su tamaño
        paginas_bloque = max(1, round(len(bloque) / densidad_objetivo))

        # Repartir las fotos de ESTE bloque en esas páginas (mismo motor que ya usamos)
        idx = 0
        while idx < len(bloque):
            restantes_bloque = len(bloque) - idx
            paginas_restantes_bloque = max(1, paginas_bloque - len([p for p in paginas if p.get("_bloque_id") == id(bloque)]))

            # Densidad para esta página concreta dentro del bloque
            n_coger = max(1, math.ceil(restantes_bloque / paginas_restantes_bloque))
            n_coger = min(n_coger, 4)  # nunca más de 4 fotos por página
...
def generar_propuestas_portada(fotos_rutas, videos_rutas, titulo_cliente="", formato="2128", orientacion="v", packs_extra=0):
    """
    ...
    # ── PIEZA 1: contar fotos (N) y calcular páginas objetivo (P) ──
    N = len(fotos)
    if N < 2:
    ...
    log(f"Fotos únicas: {N} · Páginas objetivo: {P} (30 + {packs_extra} packs) · Caso: {caso}", "📄")
...
def generar_pdf(AW, AH, diseño, fotos_dict, qr_map, ruta, titulo, subtitulo, portada_elegida=None, do_wm=False, caso_reparto="A", paginas_objetivo=30):
    ...
    # ── PRECÁLCULO PARA CASO B: páginas asignadas a cada capítulo ──
    paginas_por_capitulo = []
    if caso_reparto == "B":
