...
def _recortar_paginas_contenido_hasta_objetivo(paginas, paginas_objetivo, qr_map, AW, AH):
    ...
    páginas de contenido CONSECUTIVAS con pocas fotos entre sí (máximo 4
    juntas) hasta llegar exactamente al objetivo o quedarse sin parejas
    fusionables (todas ya al máximo de fotos por página).
    """
    MAX_FOTOS_POR_PAGINA = 4
    intentos = 0
    while intentos < paginas_objetivo * 4:
        ...
        for pos in range(len(idxs) - 1):
            i1, i2 = idxs[pos], idxs[pos + 1]
            if i2 != i1 + 1:
                continue  # no son contenido-contenido consecutivas de verdad (hay algo entre medias)
