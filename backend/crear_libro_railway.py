...
def calcular_estructura_libro(diseño, fotos_dict, qr_map, portada_elegida, caso_reparto="A", paginas_objetivo=30, AW=200, AH=200):
    ...
    if total_contenido_final != paginas_objetivo:
        # Si esto llega a saltar, es un caso realmente extremo (por
        # ejemplo, 0 o 1 fotos en todo el libro) donde ni fusionar ni
        # partir páginas puede llegar al objetivo exacto - se deja
        # constancia fuerte en el log en vez de fallar en silencio,
        # porque significa que el PDF va a tener menos o más páginas de
        # las contratadas con el cliente.
        log(f"⚠️ AVISO IMPORTANTE: el libro final tiene {total_contenido_final} páginas de "
            f"contenido en vez de las {paginas_objetivo} contratadas - revisar manualmente "
            f"antes de mandarlo a imprenta.", "!")
