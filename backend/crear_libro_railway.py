...
def _pagina_contenido_desde_calculo(pg):
    ...
    for f in pg["fotos"]:
        ...
        if ruta and os.path.exists(ruta):
            fotos_out.append({"nombre": nombre, "ruta": ruta})
    return {
        "tipo": "contenido",
        "layout": pg["layout"],
        "fotos": fotos_out,
...
def asignar_frames_video_a_capitulos(diseño, fotos_dict):
    ...
    if not any(f is not None for f in fechas_capitulo):
        return  # ningun capitulo tiene fechas de referencia, no hay con que comparar
