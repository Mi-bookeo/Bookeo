...
import os, io, sys, json, base64, datetime, re, math, uuid, struct
import time
import copy
import resource
import gc
...
FUENTES_DISPONIBLES = {
    ...
    "Butterpop": "Butterpop.ttf",
    "HighEmpathy": "High Empathy.ttf",
    "Mocaroni": "Mocaroni.ttf",
    "Patung": "Patung.ttf",
    "PinkRose": "Pink Rose.ttf",
    "SunnySpellsBasic": "Sunny Spells Basic.ttf",
    "ViksiScript": "Viksi Script.ttf",
}
...
MG = 10
GAP = 6
MARGEN_FOTO_COMPLETA = 6  # mm - margen para el layout "1" (foto a página completa).
                          # Antes 3mm - se veía casi a sangre total en el editor,
                          # como si no llevara margen ninguno. Ahora 6mm, se nota
                          # de verdad tanto en el editor como en el PDF final.
QR_MM = 22
QR_URL_PRUEBA = ""
MIN_PPI_OK = 180
MIN_PPI_BEST = 250
...
def _leer_fecha_creacion_video(ruta):
    """
    Lee la fecha de creación incrustada DENTRO del propio archivo de
    vídeo (átomo 'mvhd', dentro de 'moov', en contenedores MP4/MOV/M4V) -
    a diferencia del nombre del archivo o la fecha de modificación del
    sistema, esta fecha va incrustada en el propio vídeo y la mayoría de
    herramientas (incluida la de "Reducir/Unir vídeo" de esta misma app)
    la copian del original al generar el archivo nuevo, aunque cambien el
    nombre del archivo y la fecha de modificación pase a ser la de hoy.
    Recorre la estructura de átomos sin cargar el archivo entero en
    memoria. Devuelve None si no se encuentra o no es un MP4/MOV válido.
    """
    EPOCH_MP4 = datetime.datetime(1904, 1, 1)

    def _buscar_mvhd(f, fin, profundidad=0):
        if profundidad > 6:  # por si acaso, para no recursar sin límite en un archivo raro
            return None
        while f.tell() < fin:
            pos_atomo = f.tell()
            cabecera = f.read(8)
            ...
            tipo = tipo.decode("latin1", errors="ignore")
            if tam == 1:  # tamaño extendido (64 bits) para átomos muy grandes
                tam_ext = struct.unpack(">Q", f.read(8))[0]
                cuerpo_inicio = f.tell()
                tam_atomo_total = tam_ext
            elif tam == 0:  # el átomo ocupa hasta el final del archivo
                cuerpo_inicio = f.tell()
                tam_atomo_total = fin - pos_atomo
            else:
                cuerpo_inicio = f.tell()
                tam_atomo_total = tam

            siguiente_atomo = pos_atomo + tam_atomo_total
            if siguiente_atomo <= pos_atomo or siguiente_atomo > fin:
                return None  # tamaño inconsistente, mejor abandonar que colgarse

            if tipo in ("moov", "trak", "mdia"):
                resultado = _buscar_mvhd(f, siguiente_atomo, profundidad + 1)
                if resultado:
                    return resultado
                f.seek(siguiente_atomo)
            elif tipo == "mvhd":
                version = f.read(1)
                if not version:
                    return None
                f.read(3)  # flags, no hacen falta
                if version[0] == 1:
                    creado = struct.unpack(">Q", f.read(8))[0]
                else:
                    creado = struct.unpack(">I", f.read(4))[0]
                return EPOCH_MP4 + datetime.timedelta(seconds=creado) if creado > 0 else None
            else:
                f.seek(siguiente_atomo)
        return None
    ...
    try:
        with open(ruta, "rb") as f:
            ...
            fin = f.tell()
            f.seek(0)
            fecha = _buscar_mvhd(f, fin)
            if fecha and 2000 <= fecha.year <= 2035:
                return fecha
    except Exception:
        pass
    return None
...
def leer_fecha(ruta):
    ...
    # Para vídeos MP4/MOV, se intenta primero la fecha incrustada en el
    # propio contenedor - más fiable que el nombre del archivo o la fecha
    # de modificación, sobre todo para vídeos que han pasado por
    # "Reducir/Unir vídeo" (ese proceso genera un archivo nuevo con
    # nombre distinto y fecha de modificación de HOY, pero conserva la
    # fecha de creación real dentro del propio vídeo). Antes esto se
    # saltaba directamente al nombre de archivo/fecha de modificación
    # para cualquier vídeo, ya que Image.open() no sabe leer vídeos -
    # así que un vídeo reprocesado siempre acababa con "fecha no fiable"
    # aunque la fecha real siguiera disponible dentro del archivo.
    if ext in (".mp4", ".mov", ".m4v"):
        fecha_video = _leer_fecha_creacion_video(ruta)
        if fecha_video:
            return fecha_video, "metadata_video"
...
def calcular_ventana_recorte_frac(ruta, w_mm, h_mm, sin_deteccion_caras=False):
    """
    ...
    sin_deteccion_caras=True se usa para los fotogramas de vídeo
    (es_frame_video) - un fotograma no es una foto de verdad que el
    cliente eligió, es solo una imagen de referencia para el QR, así que
    no tiene sentido hacerle el mismo recorte "a lo cara" que a una foto
    real (podía acabar mostrando solo un primer plano de una cara al
    fondo del vídeo en vez de la escena completa). Con esto se fuerza
    siempre el recorte centrado neutro, como si no se hubiera detectado
    ninguna cara.
    """
    try:
        img = Image.open(ruta)
        ...
        caras = None if sin_deteccion_caras else detectar_caras(ruta)
...
def foto_zona(c, ruta, x, y, w, h, check_ppi=True, recorte_frac=None, contener=False):
    if not ruta or not os.path.exists(ruta):
        log(f"Foto no encontrada: {ruta}", "!")
        return
    log_memoria(f"antes de {os.path.basename(ruta)}")
    if contener:
        # Fotogramas de vídeo (es_frame_video): se ve la escena COMPLETA,
        # sin recortar nada - ni "a lo cara" ni el recorte centrado neutro
        # de siempre, que también recortaba un lado para encajar en la
        # proporción del hueco. Se ajusta la imagen entera dentro del
        # hueco (como "object-fit: contain"), pudiendo quedar un margen
        # sin foto a los lados o arriba/abajo si la proporción no coincide
        # - mejor eso que perder parte de la escena del vídeo.
        try:
            img = Image.open(ruta)
            try:
            ...
            log(f"Error abriendo fotograma {os.path.basename(ruta)}: {e}", "!")
            return
    else:
    ...
    rz = wm / hm
    if contener:
        # "contain": la imagen entera cabe dentro del hueco, sin recortar
        # (al revés que el "cover" de abajo, que recorta lo que sobre).
        if ri > rz:
            wd = wm
            hd = wm / ri
            xd = xm
...
def mapear_fuente(font_family, bold=False, italic=False):
    ...
    fuentes_custom = [
        ...
        ("butterpop", "Butterpop"),
        ("high empathy", "HighEmpathy"),
        ("mocaroni", "Mocaroni"),
        ("patung", "Patung"),
        ("pink rose", "PinkRose"),
        ("sunny spells basic", "SunnySpellsBasic"),
        ("viksi script", "ViksiScript"),
    ]
...
def _dibujar_texto_editor(c, AW, AH, canvas_w, canvas_h, info, texto_fallback):
    ...
    # Red de seguridad: si por lo que sea no hay un tamaño de referencia
    # válido para calcular las fracciones (canvas_w/canvas_h en 0 o None -
    # visto en producción al regenerar el PDF una segunda vez desde una
    # estructura ya editada), NO se debe tirar la generación del libro
    # ENTERO por un solo texto - mejor omitir justo este texto y que el
    # resto del libro se genere bien, que perder el libro completo con un
    # TypeError al dividir por None.
    if not canvas_w or not canvas_h:
        # No se ha podido aislar con certeza por qué a veces llega sin
        # tamaño de referencia (visto tras borrar un subtítulo y volver a
        # finalizar) - se vuelca aquí el contenido completo de 'info' para
        # poder diagnosticarlo de verdad la próxima vez que pase, en vez
        # de mandar el libro entero al garete por un solo texto.
        log(f"Texto de editor sin canvas_w/canvas_h válido, se omite: {texto[:40]!r} - info completa: {info!r}", "!")
        return
    ...
    # el navegador. Título/subtítulo/texto suelto de la portada y los
    # títulos de capítulo se crean SIEMPRE con origen 'center' (para que
    # sea más fácil centrarlos al arrastrar) - solo el pie de foto de las
    # páginas de contenido usa 'left' de verdad, y ese SIEMPRE guarda su
    # propio originX explícito (ver _infoEstiloTexto en editor.html), así
    # que nunca llega aquí sin el dato. Por eso 'center' es el valor por
    # defecto correcto cuando falta el dato (libros guardados antes de
    # incluir este campo) - con 'left' el texto salía descuadrado hacia la
    # izquierda en vez de centrado, que es justo lo que se veía mal.
    origin_x = info.get("originX", "center")
    origin_y = info.get("originY", "center")
...
def dibujar_foto_editor_libre(c, ruta, foto_info, canvas_w, canvas_h, aw_pt, ah_pt, clip_a_pagina=True):
    ...
    try:
        ...
        # imagen que el cliente vio en el navegador al recortar - para
        # portadas hechas con IA, esa era una MINIATURA reducida (máx.
        # 1000px, ver foto_a_base64 en main.py), no la foto a tamaño
        # completo. Si no se reescala aquí a la resolución real, el
        # recorte queda encogido a una fracción minúscula de la foto -
        # "portada ampliada". foto_ref_w/h (si viene) es el tamaño de esa
        # imagen de referencia; si no viene (fotos subidas directamente,
        # que ya se cargan a tamaño real), se usa el tamaño del propio
        # archivo como siempre.
        foto_ref_w = foto_info.get("foto_ref_w") or iw_true
        foto_ref_h = foto_info.get("foto_ref_h") or ih_true
        escala_x = iw_real / foto_ref_w if foto_ref_w else 1
        escala_y = ih_real / foto_ref_h if foto_ref_h else 1
        ...
        # OJO TEMPORAL: log para diagnosticar el caso de la portada con
        # margen no deseado - si disp_w_frac/disp_h_frac no salen en 1.0
        # (o muy cerca) para una portada "a página completa", el problema
        # está en estos números de entrada (canvas_w/canvas_h no coincide
        # con el que usó el navegador, o crop_w/scale_x no representan un
        # cover real) - no en el resto del dibujado. Quitar en cuanto se
        # resuelva.
        log(f"🔍 [foto_editor_libre] canvas={canvas_w}x{canvas_h} crop_w={crop_w} crop_h={crop_h} "
            f"scale_x={scale_x} scale_y={scale_y} disp_w_frac={disp_w_frac:.4f} disp_h_frac={disp_h_frac:.4f} "
            f"cx_frac={cx_frac:.4f} cy_frac={cy_frac:.4f}", "i")
...
def dibujar_portada_editor(c, AW, AH, ruta, titulo, subtitulo, do_wm, editor):
    ...
    color_fondo = hex_a_rgb01(editor.get("color_fondo", "#ffffff"))
    c.setFillColorRGB(*color_fondo)
...
def dibujar_cubierta_spread(c, dims, ruta_foto_portada, titulo, subtitulo, do_wm, AW, AH, editor=None):
    """
    ...
    (todo en mm, origen arriba-izquierda). 'AW, AH' es el tamaño NOMINAL
    de la página interior del libro (el mismo que usa el editor para
    calcular dónde deja el cliente la foto de portada) - necesario para
    la portada, ver más abajo.
    """
    ...
    c.saveState()
    c.translate(x0, y0)
    # OJO: la foto de portada se posicionó/recortó/escaló en el editor
    # pensando en el tamaño NOMINAL de la página interior del libro
    # (AW x AH, el mismo de siempre) - pero el panel real de portada
    # dentro del spread que exige Gelato (contentFrontSize) casi nunca
    # mide EXACTAMENTE lo mismo (suele llevar su propio margen de
    # envoltura/pegado). Antes se dibujaba directamente a las medidas de
    # contentFrontSize, así que la MISMA posición/escala que el cliente
    # dejó (pensada para encajar justa en AW x AH) caía en un marco de
    # proporción ligeramente distinta - resultado: en el editor se veía
    # la foto a sangre completa, sin márgenes, pero en el PDF final
    # aparecía con márgenes blancos alrededor. Se dibuja al tamaño
    # nominal de siempre y se escala ese resultado para que encaje
    # exactamente en el panel real de portada, conservando la misma
    # posición relativa que vio el cliente.
    escala_x = w / (AW * mm)
    escala_y = h / (AH * mm)
    c.scale(escala_x, escala_y)
    dibujar_portada(c, AW, AH, ruta_foto_portada, titulo, subtitulo, False, editor=editor)
    c.restoreState()
...
def _zonas_grande_mas_pequenas_mm(lado, n_pequenas, AW, AH, pie_h=0):
    """
    Una foto grande + N pequeñas en fila/columna, al lado que se pida
    ('izquierda', 'derecha', 'arriba', 'abajo') - generaliza lo que antes
    era un único layout fijo ("3" = grande a la izquierda, 2 pequeñas a
    la derecha) para poder tener también la versión con la grande a la
    derecha, arriba o abajo, y con 2 o 3 fotos pequeñas. Misma proporción
    de siempre para la foto grande (60% del lado correspondiente).
    """
    if lado in ("arriba", "abajo"):
        h_disponible = AH - MG * 2 - pie_h - GAP
        h_grande = h_disponible * 0.60
        h_peq_total = h_disponible - h_grande
        h_peq = (h_peq_total - GAP * (n_pequenas - 1)) / n_pequenas
        w_peq = AW - MG * 2
        y0 = MG + pie_h
        if lado == "arriba":
            y_grande, y_peq0 = y0, y0 + h_grande + GAP
        else:
            y_peq0, y_grande = y0, y0 + h_peq_total + GAP
        pequenas = [(MG, y_peq0 + i * (h_peq + GAP), w_peq, h_peq) for i in range(n_pequenas)]
        grande = (MG, y_grande, AW - MG * 2, h_grande)
        return [grande] + pequenas if lado == "arriba" else pequenas + [grande]
    else:
        w_grande = AW * 0.60 - MG
        w_peq = AW - MG - w_grande - GAP - MG
        h_zona = AH - MG * 2 - pie_h
        h_peq = (h_zona - GAP * (n_pequenas - 1)) / n_pequenas
        y0 = MG + pie_h
        if lado == "izquierda":
            x_grande, x_peq = MG, MG + w_grande + GAP
        else:
            x_peq, x_grande = MG, MG + w_peq + GAP
        pequenas = [(x_peq, y0 + i * (h_peq + GAP), w_peq, h_peq) for i in range(n_pequenas)]
        grande = (x_grande, y0, w_grande, h_zona)
        return [grande] + pequenas if lado == "izquierda" else pequenas + [grande]


def _zonas_rejilla_mm(cols, rows, AW, AH, pie_h=0):
    """Rejilla uniforme de cols x rows fotos - para las de 6, 8 y 12 por página."""
    cw = (AW - MG * 2 - GAP * (cols - 1)) / cols
    ch = (AH - MG * 2 - pie_h - GAP * (rows - 1)) / rows
    y0 = MG + pie_h
    zonas = []
    for r in range(rows):
        for col in range(cols):
            zonas.append((MG + col * (cw + GAP), y0 + r * (ch + GAP), cw, ch))
    return zonas


def _zonas_grid_mas_ancha_mm(AW, AH, pie_h=0):
    """
    Rejilla 2x2 arriba + 1 foto ancha a todo el ancho abajo, cerrando la
    página (5 fotos) - diseño clásico de photobook, visto en muestras de
    Gelato: 4 fotos en cuadrícula y una de cierre panorámica debajo.
    """
    h_total = AH - MG * 2 - pie_h - GAP
    h_grid = h_total * 0.62
    h_ancha = h_total - h_grid
    y0 = MG + pie_h
    cw = (AW - MG * 2 - GAP) / 2
    ch = (h_grid - GAP) / 2
    return [
        ...
        (MG, y0 + h_grid + GAP, AW - MG * 2, h_ancha),
    ]
...
def calcular_zonas_layout_mm(layout, num_fotos, AW, AH, pie_h=0):
    ...
    if layout in ("3", "3I"):
        return _zonas_grande_mas_pequenas_mm("izquierda", 2, AW, AH, pie_h)
    if layout == "3D":
        return _zonas_grande_mas_pequenas_mm("derecha", 2, AW, AH, pie_h)
    if layout == "3T":
        return _zonas_grande_mas_pequenas_mm("arriba", 2, AW, AH, pie_h)
    if layout == "3B":
        return _zonas_grande_mas_pequenas_mm("abajo", 2, AW, AH, pie_h)
    if layout == "4I":
        return _zonas_grande_mas_pequenas_mm("izquierda", 3, AW, AH, pie_h)
    if layout == "4D":
        return _zonas_grande_mas_pequenas_mm("derecha", 3, AW, AH, pie_h)
    if layout == "4":
        cw = (AW - MG * 2 - GAP) / 2
    ...
    if layout == "6":
        return _zonas_rejilla_mm(2, 3, AW, AH, pie_h)
    if layout == "8":
        return _zonas_rejilla_mm(2, 4, AW, AH, pie_h)
    if layout == "12":
        return _zonas_rejilla_mm(3, 4, AW, AH, pie_h)
    if layout == "5":
        return _zonas_grid_mas_ancha_mm(AW, AH, pie_h)
    # Último recurso: un layout que no se reconoce (None, o un
    # identificador de plantilla del editor sin equivalente exacto aquí,
    # ver layoutLegadoDePlantilla en editor.html). Antes esto iba borde a
    # borde sin ningún margen (y=0, alto=AH completo) - una franja vertical
    # pegada a los bordes de la página, muy distinta de cómo se ve
    # cualquier otro hueco del libro. Se respeta al menos el margen
    # estándar (MG), aunque sin garantía de que encaje con la composición
    # real que el cliente vio en el editor para esa plantilla en concreto.
    fw = (AW - MG * 2 - GAP * max(0, num_fotos - 1)) / max(1, num_fotos)
    fh = AH - MG * 2 - pie_h
    y0 = MG + pie_h
    return [(MG + i * (fw + GAP), y0, fw, fh) for i in range(num_fotos)]


def layout_generico(c, AW, AH, layout, fotos_rutas, qr_idx, qr_url, pie, do_wm, qr_pos=None, fondo=None, recortes=None, contenciones=None):
    """
    Dibuja CUALQUIER layout a partir de sus zonas (calcular_zonas_layout_mm) -
    válido tanto para los de toda la vida (1, 2H, 2V, 3, 4) como para los
    nuevos (3D/3T/3B, 4I/4D, 6, 8, 12). Una sola función en vez de una por
    diseño, para no repetir el mismo dibujado una y otra vez cada vez que
    se añade una variante nueva.
    """
    bg_blanco(c, AW, AH, fondo)
    pie_h = 8 if pie else 0
    zonas = calcular_zonas_layout_mm(layout, len(fotos_rutas), AW, AH, pie_h=pie_h)
    recortes = recortes or [None] * len(fotos_rutas)
    contenciones = contenciones or [False] * len(fotos_rutas)
    for i, r in enumerate(fotos_rutas):
        if i >= len(zonas):
            break
        x, y, w, h = zonas[i]
        rf = recortes[i] if i < len(recortes) else None
        cn = contenciones[i] if i < len(contenciones) else False
        foto_zona(c, r, x, y, w, h, recorte_frac=rf, contener=cn)
        if qr_idx == i:
            dibujar_qr_sobre_foto(c, x, y, w, h, qr_url, r, centro_custom_mm=qr_pos_a_centro_mm(qr_pos, AW, AH))
    texto_pie(c, AW, pie)
    if do_wm:
        wm(c, AW * mm, AH * mm)


def layout_1(c, AW, AH, fotos_rutas, qr_idx, qr_url, pie, do_wm, qr_pos=None, fondo=None, recortes=None, contenciones=None):
    bg_blanco(c, AW, AH, fondo)
    r = fotos_rutas[0]
    m = MARGEN_FOTO_COMPLETA
    rf = (recortes or [None])[0]
    cn = (contenciones or [False])[0]
    foto_zona(c, r, m, m, AW - m * 2, AH - m * 2, recorte_frac=rf, contener=cn)
    if qr_idx == 0:
...
def layout_2H(c, AW, AH, fotos_rutas, qr_idx, qr_url, pie, do_wm, qr_pos=None, fondo=None, recortes=None, contenciones=None):
    bg_blanco(c, AW, AH, fondo)
    pie_h = 8 if pie else 0
    fw = (AW - MG * 2 - GAP) / 2
    fh = AH - MG * 2 - pie_h
...
def layout_2V(c, AW, AH, fotos_rutas, qr_idx, qr_url, pie, do_wm, qr_pos=None, fondo=None, recortes=None, contenciones=None):
    bg_blanco(c, AW, AH, fondo)
    pie_h = 8 if pie else 0
    fh = (AH - MG * 2 - GAP - pie_h) / 2
    fw = AW - MG * 2
...
def layout_3(c, AW, AH, fotos_rutas, qr_idx, qr_url, pie, do_wm, qr_pos=None, fondo=None, recortes=None, contenciones=None):
    bg_blanco(c, AW, AH, fondo)
    pie_h = 8 if pie else 0
    pw = AW * 0.60 - MG
    ...
    contenciones = contenciones or [False, False, False]
    r0 = fotos_rutas[0]
    foto_zona(c, r0, MG, y0, pw, zh, recorte_frac=recortes[0] if len(recortes) > 0 else None,
              contener=contenciones[0] if len(contenciones) > 0 else False)
    if qr_idx == 0:
    ...
    for i, r in enumerate(fotos_rutas[1:3]):
        ...
        foto_zona(c, r, sx, y, sw, sh, recorte_frac=recortes[i + 1] if i + 1 < len(recortes) else None,
                  contener=contenciones[i + 1] if i + 1 < len(contenciones) else False)
        if qr_idx == i + 1:
...
def layout_4(c, AW, AH, fotos_rutas, qr_idx, qr_url, pie, do_wm, qr_pos=None, fondo=None, recortes=None, contenciones=None):
    bg_blanco(c, AW, AH, fondo)
    pie_h = 8 if pie else 0
    cw = (AW - MG * 2 - GAP) / 2
    ch = (AH - MG * 2 - GAP - pie_h) / 2
    ...
    contenciones = contenciones or [False, False, False, False]
...
def _dibujar_texto_titulo_capitulo(c, AW, AH, canvas_w=None, canvas_h=None,
                                    ...
                                    titulo="", subtitulo=""):
    # Los capítulos ya NO llevan subtítulo (ver comentario en
    # _dibujar_texto_titulo_capitulo_fijo) - subtitulo_info/subtitulo se
    # ignoran a propósito, aunque llegue algo guardado de antes de este
    # cambio, para que no se cuele ningún subtítulo viejo en el PDF.
    if titulo_info:
        cw = canvas_w or AW
        ch = canvas_h or AH
...
def _dibujar_texto_titulo_capitulo_fijo(c, AW, AH, titulo, subtitulo=None):
    # OJO: en reportlab el eje Y crece hacia ARRIBA (y=0 es el borde
    # inferior de la página), al revés que en el canvas del editor
    # (donde y=0 es el borde SUPERIOR). El editor pinta la banda de
    # título ARRIBA (20%) y las fotos abajo (80%) - aquí cy se calcula
    # cerca de AH (arriba del todo en reportlab), para que coincida.
    #
    # Los capítulos ya NO llevan subtítulo - solo título, centrado en toda
    # la banda (antes el título se subía un poco para dejarle sitio debajo
    # al subtítulo; ahora se centra en el medio de la banda entera). El
    # parámetro 'subtitulo' se ha dejado (ignorado) solo para no romper
    # ninguna llamada antigua que todavía se lo pase.
    banda_h = AH * 0.20
    cy = AH - (banda_h / 2)
    set_negro(c)
    c.setFont("Helvetica-Bold", 9 * mm)
    c.drawCentredString(AW * mm / 2, cy * mm, titulo)


def layout_titulo_capitulo(c, AW, AH, titulo, subtitulo, fotos_capitulo, variante, do_wm, color_fondo=None):
    """
    fotos_capitulo: lista de fotos del capítulo (dicts con "ruta" y
    "es_frame_video", o strings de ruta directamente por compatibilidad).

    Un fotograma de vídeo (es_frame_video) no se recorta EN ABSOLUTO -
    se ve completo dentro de su hueco (modo "contener"), igual que en
    todos los demás sitios del libro donde puede aparecer un fotograma.
    """
    bg_blanco(c, AW, AH, color_hex=color_fondo)
    banda_h = AH * 0.20
    foto_h = AH - banda_h - MG
    foto_y = MG  # las fotos empiezan justo encima del margen inferior; la banda del título queda arriba del todo
    _dibujar_texto_titulo_capitulo(c, AW, AH, titulo=titulo, subtitulo=subtitulo)

    def _ruta(f):
        return f["ruta"] if isinstance(f, dict) else f

    def _es_frame(f):
        return isinstance(f, dict) and f.get("es_frame_video", False)

    n = len(fotos_capitulo)
    if n == 0:
        pass
    elif n == 1:
        w_z, h_z = AW - MG * 2, foto_h
        f = fotos_capitulo[0]
        foto_zona(c, _ruta(f), MG, foto_y, w_z, h_z, contener=_es_frame(f))
    elif variante % 2 == 0 or n == 2:
        fw = (AW - MG * 2 - GAP) / 2
        for j, f in enumerate(fotos_capitulo[:2]):
            foto_zona(c, _ruta(f), MG + j * (fw + GAP), foto_y, fw, foto_h, contener=_es_frame(f))
    else:
        w_z, h_z = AW - MG * 2, foto_h
        f = fotos_capitulo[0]
        foto_zona(c, _ruta(f), MG, foto_y, w_z, h_z, contener=_es_frame(f))
...
def elegir_layout(fotos_grupo, layout_anterior="", variante=0):
    ...
    if n == 2:
        if layout_anterior == "2H":
            ...
            return "2V", fotos_grupo
    if n == 3:
        # OJO: "3T"/"3B" (grande arriba/abajo) quedaban fuera de esta
        # lista - en esa variante las 2 fotos pequeñas van APILADAS A TODO
        # EL ANCHO de la página (ver _zonas_grande_mas_pequenas_mm, rama
        # "arriba"/"abajo"), no una al lado de la otra en una columna
        # estrecha como en "3I"/"3D" - el resultado son dos tiras
        # horizontales muy bajas que quedan mal con la mayoría de fotos.
        # Se deja "3T"/"3B" disponible para que el cliente lo elija a
        # mano si quiere (sigue estando en el panel de Diseños del
        # editor), pero ya no lo elige la IA sola al montar el libro.
        opciones = [o for o in ("3I", "3D") if o != layout_anterior]
        if not opciones:
            opciones = ["3I", "3D"]
        return opciones[variante % len(opciones)], fotos_grupo
    if n == 12:
        return "12", fotos_grupo[:12]
    if n == 8:
        return "8", fotos_grupo[:8]
    if n == 6:
        return "6", fotos_grupo[:6]
    if n == 5:
        return "5", fotos_grupo[:5]
    if n >= 4:
        if layout_anterior == "4":
            if variante % 2 == 0:
                return "3I", fotos_grupo[:3]
            else:
                return "2H", fotos_grupo[:2]
        # De vez en cuando, en vez de la rejilla 2x2 de siempre, una
        # versión asimétrica (1 grande + 3 pequeñas) para dar algo más de
        # variedad sin dejar de ser un diseño sencillo.
        if variante % 3 == 2:
            return ("4I" if variante % 2 == 0 else "4D"), fotos_grupo[:4]
...
def paginas_para_capitulo_caso_b(fotos_capitulo, paginas_asignadas, qr_map, texto_titulo="", variante_inicio=0):
    ...
    for bloque in bloques:
        ...
        while idx < len(bloque):
            ...
            # Antes esto se limitaba siempre a 4 como máximo - si un
            # bloque (fotos del mismo día) tenía muchas fotos y poco
            # presupuesto de páginas, se quedaban fotos sin colocar. Ahora
            # puede escalar a un diseño más denso (6, 8, muy raramente 12)
            # cuando de verdad hace falta, igual que el resto del sistema.
            n_coger = _snap_tam_grupo(min(n_coger, restantes_bloque))
...
def _snap_tam_grupo(n):
    """
    Redondea hacia abajo al tamaño de grupo de fotos más cercano que
    sabemos dibujar (1,2,3,4,6,8,12) - evita acabar con un n_coger "raro"
    (5,7,9,10,11) que elegir_layout no sabría aprovechar entero (dejaría
    fotos sueltas sin colocar).
    """
    for tam in (12, 8, 6, 5, 4, 3, 2, 1):
        if n >= tam:
            return tam
    return 1


def _tam_grupo_optimo(fotos_restantes, paginas_restantes):
    """
    Decide cuántas fotos meter en la próxima página dentro de un
    presupuesto de páginas ya fijado. Por defecto pequeño (1-4, como
    siempre) - solo escala a diseños más densos (6, luego 8, y como
    último recurso 12) cuando de verdad hay muchas más fotos que páginas
    disponibles, para no tener que descartar ninguna foto del cliente.
    12 se reserva para el caso extremo (ni con 8 por página cabrían
    todas) - "casi nunca", tal cual se pidió.
    """
    if paginas_restantes <= 0:
        return 0
    promedio = fotos_restantes / paginas_restantes
    return _snap_tam_grupo(min(math.ceil(promedio), fotos_restantes))
...
def paginas_con_tope_estricto(fotos, paginas_max, qr_map, variante_inicio=0):
    ...
    # Antes el máximo absoluto por página era 4 fotos, así que si sobraban
    # más fotos de las que cabían así, se DESCARTABAN sin más (muestreo).
    # Ahora el tope real es 12 (con _tam_grupo_optimo decidiendo cuándo
    # hace falta subir de verdad a 6/8/12) - así prácticamente nunca hay
    # que perder ninguna foto del cliente por falta de páginas.
    MAX_FOTOS_POR_PAGINA = 12
    cupo_max = paginas_max * MAX_FOTOS_POR_PAGINA

    fotos_usar = fotos
    if len(fotos) > cupo_max:
        # No caben todas ni a 12 por pagina (caso extremo de verdad):
        # muestreo uniforme conservando el orden cronologico (y la
        # primera/ultima foto) para no pasarnos.
    ...
    # distintos y pocas fotos por dia), se reconstruye con reparto por
    # tamaño de grupo óptimo, que SI garantiza el límite exacto sin
    # necesidad de descartar fotos (usa diseños más densos si hace falta).
    if len(paginas) > paginas_max:
        log(f"Sin capitulos: reparto por dia se paso ({len(paginas)}/{paginas_max}) "
            f"- recalculando con reparto por tamaño óptimo", "!")
        n = len(fotos_usar)
        paginas = []
        idx = 0
        variante = variante_inicio
        layout_ant = ""
        paginas_usadas = 0
        while idx < n and paginas_usadas < paginas_max:
            tam = _tam_grupo_optimo(n - idx, paginas_max - paginas_usadas)
            if tam <= 0:
                break
...
def _completar_paginas_contenido_hasta_objetivo(paginas, paginas_objetivo, qr_map, AW, AH):
    ...
    while faltan > 0 and intentos < paginas_objetivo * 4:
        ...
        # OJO: partir una página a la mitad "a ciegas" (por posición, sin
        # mirar qué hay en cada mitad) puede dejar un fotograma de vídeo
        # SOLO en una de las dos mitades - justo lo que _evitar_fotograma_
        # solo() se encarga de evitar. Como esta función se llama tanto
        # antes como (ahora) después de esa, podía deshacer su trabajo:
        # colocaba bien el fotograma acompañado, y el siguiente reparto
        # por huecos lo volvía a dejar solo. Se prefieren páginas SIN
        # fotograma para partir; si no queda ninguna, se parte una CON
        # fotograma pero forzando que se quede en el lado con más fotos
        # (nunca solo).
        sin_frame = [i for i in candidatos
                     if not any(f.get("es_frame_video") for f in paginas[i].get("fotos", []))]
        # Una página con el fotograma y solo 1 foto más (2 en total) NUNCA
        # se puede partir sin dejarlo solo (1+1) - se excluye del todo de
        # los candidatos "de emergencia"; solo se acepta partir una página
        # con fotograma si tiene 3 o más fotos, así siempre queda margen
        # para que el lado del fotograma tenga compañía.
        con_frame_seguro = [i for i in candidatos
                             if i not in sin_frame and len(paginas[i].get("fotos", [])) >= 3]
        usar = sin_frame if sin_frame else con_frame_seguro
        if not usar:
            break  # solo quedan páginas de 1 foto, o de 2 con el fotograma - no partibles
        idx = max(usar, key=lambda i: len(paginas[i]["fotos"]))
        pagina = paginas[idx]
        fotos = pagina["fotos"]
        ...
        idx_frame = next((k for k, f in enumerate(fotos) if f.get("es_frame_video")), None)
        if idx_frame is not None:
            # Si el corte a mitad de camino dejaría al fotograma solo en
            # su lado, se mueve una foto de más del otro lado para que
            # nunca quede solo - a costa de un reparto algo menos parejo
            # entre las dos páginas resultantes, que es preferible.
            frame_en_a = idx_frame < mitad
            lado_frame = fotos_a if frame_en_a else fotos_b
            lado_otro = fotos_b if frame_en_a else fotos_a
            if len(lado_frame) == 1 and len(lado_otro) >= 2:
                if frame_en_a:
                    fotos_a = fotos_a + [lado_otro.pop(0)]
                    fotos_b = lado_otro
                else:
                    fotos_b = fotos_b + [lado_otro.pop(0)]
                    fotos_a = lado_otro
    ...
    # Último recurso: si ya no queda NINGUNA página partible (caso
    # extremo, con muy pocas fotos - ej. un pedido con menos fotos que
    # páginas contratadas) y aun así faltan páginas, se rellena con
    # páginas de contenido vacías (sin foto, solo fondo/diseño). El
    # número de páginas contratado es un compromiso comercial con Gelato
    # y con el cliente - tiene que cumplirse SIEMPRE, aunque el contenido
    # de esas páginas de más no pueda ser una foto de verdad.
    while faltan > 0:
        paginas.append({
            "tipo": "contenido", "layout": "1", "fotos": [], "qr_idx": -1, "qr_url": "",
            "texto": "", "capitulo": None,
        })
        faltan -= 1
...
def _recortar_paginas_contenido_hasta_objetivo(paginas, paginas_objetivo, qr_map, AW, AH):
    """
    Simétrica a _completar_paginas_contenido_hasta_objetivo: si el total de
    páginas de contenido se pasa del objetivo (por ejemplo, un caso límite
    en el que las fotos sobrantes forzaban su propia página aunque los
    capítulos ya hubieran agotado el presupuesto), se fusionan parejas de
    páginas de contenido con pocas fotos entre sí (máximo 4 juntas) hasta
    llegar exactamente al objetivo o quedarse sin parejas fusionables
    (todas ya al máximo de fotos por página).

    OJO: antes solo se fusionaban páginas ADYACENTES (una justo detrás de
    otra en el propio libro) - con muchos capítulos pequeños (uno con una
    sola página de contenido cada uno, separados por su título de
    capítulo) nunca hay dos páginas de contenido seguidas de verdad, así
    que nunca encontraba nada que fusionar y el libro se quedaba siempre
    por encima del objetivo (31 en vez de 30). Ahora no hace falta que
    estén una al lado de la otra - se prioriza fusionar dentro del MISMO
    capítulo (para no mezclar fotos de momentos distintos), y si no hay
    ninguna pareja así, cualquier par de páginas de contenido sirve.
    """
    # Con los diseños densos ya disponibles (6, 8, 12), fusionar hasta 12
    # fotos juntas es válido - antes esto se limitaba a 4, lo que hacía
    # más fácil quedarse sin ninguna pareja fusionable disponible.
    MAX_FOTOS_POR_PAGINA = 12
    intentos = 0
    while intentos < paginas_objetivo * 4:
        intentos += 1
        idxs = [i for i, p in enumerate(paginas) if p.get("tipo") == "contenido"]
        if len(idxs) <= paginas_objetivo:
            break

        # Dos pasadas: primero se intenta SIN pasarse del límite normal de
        # fotos por página (mejor calidad de maquetación); si ninguna
        # pareja cabe así (caso extremo con páginas ya muy llenas), se
        # repite sin ese límite - el objetivo de página es un compromiso
        # comercial (lo pactado con el cliente) y tiene MÁS prioridad que
        # mantener el límite estético de fotos por página. Nunca debe
        # quedar el libro por encima de paginas_objetivo mientras exista
        # al menos una pareja de páginas de contenido para fusionar.
        mejor_pareja = None
        for limite in (MAX_FOTOS_POR_PAGINA, None):
            for a in range(len(idxs)):
                i1 = idxs[a]
                fotos1 = paginas[i1].get("fotos", [])
                for b in range(a + 1, len(idxs)):
                    i2 = idxs[b]
                    fotos2 = paginas[i2].get("fotos", [])
                    if limite is not None and len(fotos1) + len(fotos2) > limite:
                        continue
                    mismo_capitulo = paginas[i1].get("capitulo") == paginas[i2].get("capitulo")
                    if mejor_pareja is None:
                        mejor_pareja = (i1, i2)
                    if mismo_capitulo:
                        mejor_pareja = (i1, i2)
                        break
                if mejor_pareja and paginas[mejor_pareja[0]].get("capitulo") == paginas[mejor_pareja[1]].get("capitulo"):
                    break
            if mejor_pareja:
                break

        if not mejor_pareja:
            # Esto solo puede pasar con 0 o 1 páginas de contenido en todo
            # el libro (nada que fusionar) - no hay pareja posible.
            log(f"No se pudo recortar más páginas de contenido ({len(idxs)}/{paginas_objetivo}) "
                f"- no queda ninguna pareja de páginas de contenido para fusionar", "!")
            break

        i1, i2 = mejor_pareja
        fotos1, fotos2 = paginas[i1].get("fotos", []), paginas[i2].get("fotos", [])
        grupo = fotos1 + fotos2
        layout, fotos_layout = elegir_layout(grupo)
        qr_idx, qr_url = -1, ""
        for fi, foto in enumerate(fotos_layout):
            if foto.get("nombre") in qr_map:
                ...
                break
        pg = {"layout": layout, "fotos": fotos_layout, "qr_idx": qr_idx, "qr_url": qr_url,
              "texto": paginas[i1].get("texto") or paginas[i2].get("texto") or ""}
        nueva = _pagina_contenido_desde_calculo(pg, AW, AH)
        nueva["capitulo"] = paginas[i1].get("capitulo") or paginas[i2].get("capitulo")
        # i2 siempre es mayor que i1 (viene de b > a) - se borra primero
        # el de índice más alto para no descuadrar la posición del otro.
        del paginas[i2]
        paginas[i1] = nueva
...
def analizar_con_ia(fotos, dias, titulo_cliente="", sin_capitulos=False):
    ...
    if len(fotos) <= MAX:
        ...
        # Se rellenan los huecos que sobren con fotos fiables - pero
        # repartidas por BLOQUE DE DÍA, no con una zancada uniforme ciega
        # sobre la lista entera. Antes, un día con muchas fotos seguidas
        # (p.ej. 40 fotos de un bautizo) podía quedarse SIN NINGUNA foto
        # visible para la IA si la zancada uniforme caía siempre fuera de
        # ese tramo - así la IA no tenía ninguna manera de detectar de qué
        # evento se trataba, por muy claro que fuera con solo ver 1 foto.
        # Ahora se reparte por turnos entre los bloques de día, empezando
        # por los más grandes (más fotos seguidas = más probable que sea
        # un evento importante que merece al menos una foto de
        # referencia), así que ningún día con muchas fotos se queda a
        # ciegas mientras haya huecos para darle al menos una.
        muestra = list(fotos_dudosas)
        huecos_libres = MAX - len(muestra)
        if huecos_libres > 0 and fotos_fiables:
            fotos_fiables_ordenadas = sorted(fotos_fiables, key=lambda f: f["fecha"])
            bloques_fiables = agrupar_por_dia(fotos_fiables_ordenadas)
            restantes = {i: list(b) for i, b in enumerate(bloques_fiables)}
            orden_por_tamano = sorted(restantes.keys(), key=lambda i: -len(restantes[i]))
            elegidas = []
            while len(elegidas) < huecos_libres and any(restantes.values()):
                avanzo = False
                for i in orden_por_tamano:
                    if len(elegidas) >= huecos_libres:
                        break
                    bloque = restantes[i]
                    if bloque:
                        # La del medio del tramo que queda suele ser más
                        # representativa que la primera/última (llegada o
                        # recogida, menos "del evento en sí").
                        elegidas.append(bloque.pop(len(bloque) // 2))
                        avanzo = True
                if not avanzo:
                    break
            muestra += elegidas
...
- viaje: agrupa por dia o destino - el titulo de cada capitulo tiene que describir el lugar o lo que se ve en las fotos (ej: "Llegada a Roma", "Playa y atardecer", "El Coliseo", "Cena en el puerto") - NUNCA titules un capitulo "Dia 1", "Dia 2", etc., ni ningun otro numero o fecha sueltos como titulo
...
RESPONDE SOLO JSON compacto:
{{"tipo":"bebe","tipo_desc":"primer ano de vida","titulo":"Catalina","subtitulo":"Febrero 2023 - Enero 2024","portada_opciones":[{{"foto":"foto1.jpg","titulo":"Catalina","subtitulo":"Febrero 2023 - Enero 2024"}},{{"foto":"foto5.jpg","titulo":"Catalina","subtitulo":"Su primer ano de vida"}}],"capitulos":[{{"titulo":"Mes 1","fotos":["foto1.jpg","foto2.jpg"]}}]}}

REGLAS:
- Los capitulos NO llevan subtitulo, solo "titulo" y "fotos" (el ejemplo de arriba es el formato exacto) - el subtitulo del campo general del libro y de portada_opciones sigue haciendo falta como siempre, esto es solo para los capitulos
- Mira bien las fotos para identificar de que evento o lugar concreto se trata (una tarta con velas, un traje de bautizo, un cartel con el nombre de un sitio, decoracion de una fiesta en concreto...) y usa eso para poner titulos de capitulo especificos y personales - "El bautizo de Marta", "Cumpleanos en el jardin", "La Sagrada Familia" - en vez de titulos genericos o vacios como "Capitulo 1", "Dia 3" o solo una fecha
...
- Maximo 2 fotos por capitulo si solo tienes 2"""
    ...
    for intento in range(3):
        try:
            # OJO: antes esto era cli.messages.create(...) SIN streaming,
            # esperando de golpe una respuesta larga (hasta 8000 tokens,
            # con varios capítulos y listas de fotos dentro). Con
            # peticiones así de largas sin streaming, es fácil que algún
            # proxy intermedio (Railway, Cloudflare...) corte la conexión
            # por inactividad si pasan muchos segundos sin recibir ni un
            # byte de respuesta - para el cliente eso se ve como que la
            # pantalla de "generando propuestas" se queda pensando sin
            # avanzar nunca, en vez de fallar y reintentar. Con streaming
            # llegan fragmentos de texto sin parar durante toda la
            # generación, así ningún proxy la corta por creerla muerta.
            # timeout=90 además evita que un intento colgado se quede
            # esperando indefinidamente antes de pasar al reintento.
            texto_completo = []
            ...
            if intento == 2:
                raise
            # Pequeña espera antes de reintentar - si el fallo fue por
            # sobrecarga puntual de la API (rate limit, 529 overloaded),
            # repetir la petición al instante sin esperar nada suele
            # volver a chocar con el mismo problema.
...
def calcular_estructura_libro_vacia(portada_elegida, paginas_objetivo=30, AW=200, AH=200, fotos_dict=None):
    """
    ...
    """
    paginas = []
    fotos_dict = fotos_dict or {}

    # OJO: antes solo se resolvía el caso de "foto personalizada subida al
    # momento" (foto_personalizada_ruta) - faltaba el caso, tan común como
    # ese, de elegir como portada una foto YA subida de las del álbum
    # (portada_elegida["foto"], un nombre que hay que buscar en
    # fotos_dict para sacar su ruta real). Sin esta rama, "fotos_portada"
    # se quedaba vacío y la portada perdía su foto en la miniatura del
    # panel de páginas (y, potencialmente, en el PDF final).
    portada_nombre = ""
    portada_ruta = ""
    ...
    paginas.append({"tipo": "blanco", "layout": None, "fotos": [], "texto": "",
                     "qr_idx": -1, "qr_url": "", "capitulo": None})
    # OJO: ya no se añade aquí una página "contraportada" aparte. Nunca se
    # dibuja como página de imprenta (dibujar_pdf_desde_estructura la salta
    # con un simple "continue" - su contenido real ya sale dentro del
    # spread de la portada) y solo servía para que el editor la enseñara
    # como una página más que en realidad no existía en el libro impreso -
    # el cliente veía 34 páginas en el editor en vez de las 33 reales.
...
def calcular_estructura_libro(diseño, fotos_dict, qr_map, portada_elegida, caso_reparto="A", paginas_objetivo=30, AW=200, AH=200):
    ...
    for idx_cap, cap in enumerate(diseño.get("capitulos", [])):
        ...
        # Antes se cogían SIEMPRE 2 fotos para la página de título del
        # capítulo, sin mirar cuántas tenía el capítulo en total - en uno
        # con pocas fotos, "gastarse" 2 solo en el título dejaba muy poco
        # para las páginas de contenido de después. Ahora solo se cogen 2
        # si el capítulo tiene fotos de sobra (6 o más en total); si no,
        # se coge solo 1 y las demás quedan libres para el contenido.
        n_para_titulo = 2 if len(fotos_cap) >= 6 else 1
        fotos_titulo = fotos_cap[:n_para_titulo]
        ...
        if fotos_resto:
            if caso_reparto == "B":
                # OJO: mismo bug que el de las fotos sobrantes, pero AQUÍ
                # dentro de cada capítulo - "paginas_por_capitulo[idx_cap]"
                # ya incluye la página del título; si a un capítulo pequeño
                # le tocó justo 1 página en el reparto (solo el título, sin
                # contenido), "- 1" da 0, pero el "max(1, ...)" de antes
                # forzaba SIEMPRE al menos 1 página de contenido igualmente
                # - una página de más por CADA capítulo pequeño así, sin
                # que nada la compensara. Con varios capítulos pequeños
                # (típico cuando la IA organiza por días/momentos) esto se
                # acumulaba y el libro se pasaba del objetivo (31, a veces
                # más). Ahora se deja en 0 si así lo dice el reparto - si
                # aun así hacen falta páginas para no perder fotos, la red
                # de seguridad de después ya se encarga de que el TOTAL
                # del libro cuadre exacto, quitando de donde sobre.
                paginas_asignadas_cap = max(0, paginas_por_capitulo[idx_cap] - 1)
                paginas_calc = paginas_con_tope_estricto(
                    fotos_resto, paginas_asignadas_cap, qr_map, variante_inicio=capitulo_variante
                )
                if not paginas_calc:
                    # No había presupuesto para este capítulo pero SÍ hay
                    # fotos suyas sin usar - no se pueden perder fotos del
                    # cliente sin más. Se les da 1 página como mínimo vital
                    # (no 0), confiando en que el recorte final compensará
                    # el excedente en otro capítulo con más margen.
                    paginas_calc = paginas_con_tope_estricto(
                        fotos_resto, 1, qr_map, variante_inicio=capitulo_variante
                    )
            else:
    ...
    # -- FOTOS SOBRANTES --
    # Red de seguridad para el caso límite, no el camino normal: los
    # fotogramas de vídeo YA se asignan a su capítulo real por fecha (o
    # por imagen si la fecha no es de fiar) en asignar_frames_video_a_capitulos(),
    # que se llama ANTES de esta función - así que, salvo error, aquí no
    # debería caer ningún fotograma ni ninguna foto normal. Solo entran
    # en este grupo de "sobrantes" al final del libro: el modo "sin
    # capítulos" (diseño["capitulos"] vacío, donde no hay a qué capítulo
    # asignar nada) o el caso límite de que la asignación por fecha/IA no
    # encontrara ningún capítulo candidato.
    ...
    if fotos_sobrantes:
        ...
        if caso_reparto == "B":
            ...
            # El "max(1, ...)" de antes forzaba SIEMPRE al menos 1 página
            # para las sobrantes aunque los capítulos ya hubieran agotado
            # el presupuesto entero (0 páginas libres) - eso era justo lo
            # que colaba una página de más (31 en vez de 30). Ahora, si no
            # queda presupuesto, no se fuerza ninguna página extra aquí;
            # la red de seguridad de después ya se encarga de najar el
            # total a la cifra exacta si hiciera falta.
            paginas_usadas_hasta_ahora = len(paginas) - 2  # -2: portada y guarda en blanco
            paginas_restantes = max(0, paginas_objetivo - paginas_usadas_hasta_ahora)
            paginas_calc = paginas_con_tope_estricto(
    ...
    #
    # OJO IMPORTANTE: "paginas_objetivo" (30, o 30+packs) es el número
    # TOTAL de páginas interiores del libro - título de capítulo incluido,
    # no solo las de "contenido". Antes _completar_.../_recortar_... solo
    # miraban páginas "tipo=contenido" y las forzaban a paginas_objetivo
    # SIN CONTAR las de título de capítulo, así que un libro con capítulos
    # acababa con paginas_objetivo + (nº de capítulos) páginas en total -
    # con 4 capítulos, un cliente que pactó "30" se encontraba con 30 de
    # contenido + 4 de título = 34 páginas de verdad (37 contando portada/
    # guardas), sin haber tocado nada él. Se descuenta aquí el número de
    # páginas de título ya decidido por la IA, así el objetivo de
    # CONTENIDO pasa a ser el que hace que el TOTAL cuadre en 30 de
    # verdad.
    n_titulos_capitulo = sum(1 for p in paginas if p.get("tipo") == "titulo_capitulo")
    objetivo_contenido = max(1, paginas_objetivo - n_titulos_capitulo)
    paginas = _completar_paginas_contenido_hasta_objetivo(paginas, objetivo_contenido, qr_map, AW, AH)
    # -- Y su opuesta: tampoco pasarse, si por lo que sea se coló alguna
    # página de más (ver comentario en fotos_sobrantes más arriba) --
    paginas = _recortar_paginas_contenido_hasta_objetivo(paginas, objetivo_contenido, qr_map, AW, AH)
    ...
    # -- CONTRAPORTADA --
    # Ya NO se añade como página aparte. Antes se mantenía solo para que
    # el editor la enseñara (portada, blanco, contenido, blanco,
    # contraportada - 34 páginas en total en el editor), pero para el PDF
    # final de imprenta esta página NUNCA se dibujaba aparte: su
    # contenido se fusiona con la portada y el lomo en un único spread de
    # cubierta con las medidas de Gelato (ver dibujar_cubierta_spread /
    # dibujar_pdf_desde_estructura, que se saltaba este tipo a
    # propósito). El libro real son 33 páginas (portada, blanco, 30 de
    # contenido + título, blanco) - ahora el editor enseña ese mismo
    # número, sin una página fantasma de más que nunca llegaba a
    # imprimirse.

    _evitar_fotograma_solo(paginas, AW, AH)

    # -- RE-VERIFICACIÓN FINAL, OBLIGATORIA --
    # _evitar_fotograma_solo puede cambiar el número de páginas de
    # contenido (cuando el fotograma estaba solo en su propia página, esa
    # página se ELIMINA al fusionarlo en otra) - se ejecuta el último de
    # todos, así que sin este paso el recuento ya fijado arriba podía
    # descuadrarse otra vez sin que nadie lo detectara. El número TOTAL de
    # páginas es un compromiso comercial con el cliente (y con lo que se
    # paga a Gelato por página) - tiene que quedar en EXACTAMENTE
    # paginas_objetivo (título de capítulo incluido) pase lo que pase
    # antes, así que se repite la misma pareja de funciones (completar si
    # faltan / recortar si sobran) al final del todo, no solo antes de
    # tocar el fotograma. _completar_.../_recortar_... solo tocan páginas
    # "tipo=contenido" - no afectan a la guarda en blanco, a los títulos
    # de capítulo (que no cambian de número aquí) ni a la contraportada.
    n_titulos_capitulo = sum(1 for p in paginas if p.get("tipo") == "titulo_capitulo")
    objetivo_contenido = max(1, paginas_objetivo - n_titulos_capitulo)
    paginas = _completar_paginas_contenido_hasta_objetivo(paginas, objetivo_contenido, qr_map, AW, AH)
    paginas = _recortar_paginas_contenido_hasta_objetivo(paginas, objetivo_contenido, qr_map, AW, AH)
    total_interior_final = sum(1 for p in paginas if p.get("tipo") in ("contenido", "titulo_capitulo"))
    if total_interior_final != paginas_objetivo:
        # No debería poder pasar nunca - _completar_.../_recortar_... ya
        # garantizan el número exacto entre los dos (completar ahora
        # incluso rellena con páginas vacías como último recurso si no
        # queda nada que partir). Si aun así ocurriera, NO se para la
        # generación del libro (eso significaría no entregar nada al
        # cliente) - se deja un aviso bien visible en el log para
        # revisarlo, y se sigue adelante con lo que haya.
        log(f"⚠️ AVISO IMPORTANTE: el libro quedó con {total_interior_final} páginas "
            f"interiores (contenido + título de capítulo) en vez de las {paginas_objetivo} "
            f"contratadas - revisar el pedido a mano antes de mandarlo a Gelato.", "!")
...
def _evitar_fotograma_solo(paginas, AW, AH):
    ...
    con otras 2-3 fotos como mínimo (layout "3" o "4"). Cubre dos casos:
    ...
    En los dos casos se saca el fotograma de ahí y se fusiona con otra
    página de contenido que YA tenga 2 o 3 fotos (así el resultado es una
    de 3 o 4 - nunca una de 2, que volvería a ser media página). SOLO se
    busca dentro del mismo bloque del fotograma (su capítulo, si lo
    tiene, o su bloque de fecha/hora si es de las fotos "sobrantes" sin
    capítulo) - nunca en otro capítulo del libro, para no acabar
    poniendo el fotograma junto a fotos de un momento completamente
    distinto solo por encajar mejor el hueco. Dentro de ese bloque se
    busca primero cerca (páginas contiguas) y, si hace falta, más lejos -
    pero sin salir de él. Solo si NO existe ninguna página con 2-3 fotos
    en TODO ese bloque (caso límite: capítulo/bloque con muy pocas fotos)
    se acepta como último recurso fusionar con una de 1 sola foto - pero
    nunca se deja al fotograma solo del todo. Si en el caso 2 queda una
    foto normal sola en la página original, esa se queda a página
    completa (layout "1"), que es justo donde SÍ debe estar una foto de
    verdad.
    """
    LAYOUT_POR_N = {2: "2H", 3: "3", 4: "4"}

    # Fronteras de capítulo: cada página "titulo_capitulo" abre un bloque
    # nuevo. Las páginas de "fotos sobrantes" (sin capítulo, agrupadas
    # solo por fecha/hora) van todas seguidas al final, después del
    # último capítulo real, sin ningún separador propio - caen dentro del
    # mismo bloque que ese último capítulo, que es lo más parecido a "su
    # propio bloque de fecha/hora" que se puede inferir aquí sin repetir
    # el cálculo de fechas.
    fronteras = [idx for idx, p in enumerate(paginas) if p.get("tipo") == "titulo_capitulo"]
    ...
    def _buscar_destino_idx(i, pg, min_fotos, rango):
        # Busca por distancia creciente desde i (primero lo más cercano),
        # sin salir de [rango[0], rango[1]) - el bloque (capítulo o
        # tramo de fecha/hora) al que pertenece el fotograma.
        lo, hi = rango
        max_d = max(i - lo, hi - 1 - i)
        for d in range(1, max_d + 1):
            for j in (i - d, i + d):
                if lo <= j < hi:
                    candidata = paginas[j]
                    if (candidata is not pg and candidata.get("tipo") == "contenido"
                            and min_fotos <= len(candidata.get("fotos", [])) < 4):
                        return j
        return None
    ...
    while i < len(paginas):
        ...
        rango = _rango_bloque(i)

        # Primero se intenta un destino que YA tenga 2 o 3 fotos (el
        # fotograma sería la 3ª o 4ª => layout "3" o "4", un cuarto de
        # página). Solo si no hay ninguno en ese mismo bloque, se acepta
        # uno con 1 sola foto (layout "2H"/"2V", media página - peor pero
        # mejor que dejarlo solo del todo).
        destino_idx = _buscar_destino_idx(i, pg, min_fotos=2, rango=rango)
        if destino_idx is None:
            destino_idx = _buscar_destino_idx(i, pg, min_fotos=1, rango=rango)

        if destino_idx is None:
            i += 1
            continue
        destino = paginas[destino_idx]
        ...
        # OJO: ya no hace falta recalcular ningún recorte del fotograma
        # aquí - un fotograma de vídeo se dibuja siempre en modo
        # "contener" (ver foto_zona/dibujar_pdf_desde_estructura), que
        # muestra la escena completa sin recortar nada, sea cual sea el
        # tamaño del hueco que le toque.
...
def _pagina_contenido_desde_calculo(pg, AW, AH):
    ...
    for i, foto in enumerate(fotos_out):
        if i < len(zonas):
            ...
            # OJO: ESTA es la ÚNICA función que calcula el recorte de
            # verdad para toda la estructura del libro (tanto lo que ve
            # el cliente en vivo en el editor como lo que luego dibuja el
            # PDF, que reutiliza este mismo valor salvo que el cliente lo
            # toque a mano). Los fotogramas de vídeo (es_frame_video) NO
            # llevan recorte - se dibujan siempre en modo "contener"
            # (escena completa, sin recortar nada), así que no hace falta
            # calcularles ninguna ventana.
...
def dibujar_pdf_desde_estructura(paginas, AW, AH, ruta, titulo, subtitulo, portada_elegida=None, do_wm=False, formato="2128"):
    ...
    # OJO TEMPORAL: mismo diagnóstico que en dibujar_foto_editor_libre -
    # para ver los datos EN CRUDO que llegaron del editor para esta
    # portada en concreto (foto, banda, marco), antes de que se les
    # aplique ningún cálculo. Quitar en cuanto se resuelva.
    if editor_portada:
        log(f"🔍 [portada] foto={editor_portada.get('foto')} banda={editor_portada.get('banda')} "
            f"marco={editor_portada.get('marco')} canvas={editor_portada.get('canvas_w')}x{editor_portada.get('canvas_h')}", "i")
    ...
    for pagina in paginas:
        ...
        if tipo == "portada":
            ...
            dibujar_cubierta_spread(c_cubierta, dims, ruta_foto, titulo, subtitulo, do_wm, AW, AH, editor=editor_portada)
            c_cubierta.showPage()
        ...
        if tipo == "blanco":
            ...
            #
            # OJO: faltaba el caso de cambiar SOLO la fuente/color del
            # título o subtítulo, sin tocar ninguna foto ni sticker - antes
            # eso NO activaba modo_libre, así que caía en el "else" de
            # abajo (layout_titulo_capitulo), que llama a
            # _dibujar_texto_titulo_capitulo() sin pasarle titulo_info/
            # subtitulo_info en absoluto - el cambio de fuente se
            # guardaba bien en el editor pero se ignoraba por completo en
            # el PDF final. Ahora también cuenta como motivo para entrar
            # en modo_libre.
            modo_libre = (any(f.get("editor_foto") for f in fotos_cap) or bool(pagina.get("stickers"))
                          or bool(cap.get("titulo_info")) or bool(cap.get("subtitulo_info")))

            if modo_libre:
                canvas_w = pagina.get("canvas_w") or AW
                canvas_h = pagina.get("canvas_h") or AH
                aw_pt, ah_pt = AW * mm, AH * mm
                bg_blanco(c, AW, AH, color_hex=pagina.get("fondo"))

                # OJO: antes, la foto "hermana" que el cliente NO había
                # tocado (sin editor_foto) se dibujaba siempre con un
                # hueco fijo que ocupaba TODA la zona de fotos de la
                # página (ancho completo) - correcto si solo hay 1 foto,
                # pero con 2 fotos (una a cada lado) esa foto sin tocar
                # tapaba la página entera, incluida la que sí tenía
                # posición guardada (justo lo que pasaba al pellizcar
                # solo una de las dos: la otra "invadía" toda la página).
                # Ahora se calcula la mitad exacta que le toca según su
                # posición (izquierda/derecha), igual que layout_titulo_capitulo.
                banda_h_fallback = AH * 0.20
                foto_h_fallback = AH - banda_h_fallback - MG
                foto_y_fallback = MG
                n_fotos_cap = len(fotos_cap)

                def _zona_fallback_titulo(idx_f):
                    if n_fotos_cap == 2:
                        fw = (AW - MG * 2 - GAP) / 2
                        return (MG + idx_f * (fw + GAP), foto_y_fallback, fw, foto_h_fallback)
                    return (MG, foto_y_fallback, AW - MG * 2, foto_h_fallback)

                def _dibujar_fallback_titulo(idx_f, f):
                    # OJO: aquí ya se calculaba bien la mitad (zona) que le
                    # tocaba a la foto sin tocar, pero se llamaba a
                    # foto_zona() sin recorte_frac - así que para el
                    # RECORTE seguía cayendo en la detección de caras por
                    # defecto. Un fotograma de vídeo (es_frame_video) no se
                    # recorta EN ABSOLUTO (ni a lo cara ni centrado): se ve
                    # completo dentro de su mitad (contener=True), igual
                    # que en las páginas de "contenido".
                    x, y, w, h = _zona_fallback_titulo(idx_f)
                    foto_zona(c, f["ruta"], x, y, w, h, contener=bool(f.get("es_frame_video")))

                for idx_f, f in enumerate(fotos_cap):
                    info = f.get("editor_foto")
                    ok = False
                    if info:
                        ok = dibujar_foto_editor_libre(c, f["ruta"], info, canvas_w, canvas_h, aw_pt, ah_pt)
                    if not ok:
                        # Esta foto en concreto no la tocó el cliente pero
                        # su hermana sí (por eso modo_libre=True) - se
                        # coloca en SU mitad correspondiente, no en toda la
                        # página, para no tapar a la que sí se movió.
                        _dibujar_fallback_titulo(idx_f, f)
                ...
                # OJO: antes se le pasaban solo las RUTAS (strings), sin
                # el resto de datos de cada foto - así layout_titulo_capitulo
                # no tenía forma de saber si alguna era un fotograma de
                # vídeo (es_frame_video) y calcular su recorte sin
                # detección de caras. Ahora se le pasan las fotos
                # completas (los dicts) para que pueda hacerlo bien.
                #
                # También: antes esto pintaba SIEMPRE blanco fijo, sin mirar
                # si el cliente había elegido otro color de fondo para
                # esta página desde "Fondo" - igual que ya se arregló
                # para la portada, la banda del título de capítulo tiene
                # que seguir el mismo fondo que el resto de la página,
                # no quedarse blanca a la fuerza.
                layout_titulo_capitulo(c, AW, AH, cap.get("titulo", ""), cap.get("subtitulo", ""),
                                        fotos_cap, cap.get("variante", 0), do_wm,
                                        color_fondo=pagina.get("fondo"))
            ...
            textos_libres = pagina.get("textos_libres") or []
            modo_libre = (any(f.get("editor_foto") for f in fotos_validas) or bool(texto_info)
                          or bool(stickers_pagina) or bool(textos_libres))
            ...
            if modo_libre:
                ...
                # Igual que en "titulo_capitulo": si esta página tiene VARIAS
                # fotos y el cliente solo tocó una, la(s) que no tocó NO se
                # pueden dibujar a página completa (antes hacían m,m,AW-m*2,
                # AH-m*2 fijo) - eso las hacía taparse con la que sí se movió
                # y desordenaba qué queda delante/detrás. Se calcula la zona
                # real que le toca a cada una según el layout original de la
                # página (misma lógica que el camino "sin tocar" de más abajo).
                try:
                    zonas_fallback = calcular_zonas_layout_mm(
                        pagina.get("layout"), len(fotos_validas), AW, AH,
                        pie_h=8 if (pagina.get("texto") and not texto_info) else 0,
                    )
                except Exception:
                    zonas_fallback = []

                def _zona_fallback_contenido(idx_f):
                    if idx_f < len(zonas_fallback):
                        return zonas_fallback[idx_f]
                    m = MARGEN_FOTO_COMPLETA
                    return (m, m, AW - m * 2, AH - m * 2)

                for idx_f, f in enumerate(fotos_validas):
                    info = f.get("editor_foto")
                    ok = False
                    if info:
                        ok = dibujar_foto_editor_libre(c, f["ruta"], info, canvas_w, canvas_h, aw_pt, ah_pt)
                    if not ok:
                        # Esta foto no se tocó (o no se pudo dibujar en modo
                        # libre) pero otra cosa de la página sí (otra foto, o
                        # el texto) - se coloca en SU zona correspondiente,
                        # no en toda la página, para no tapar a la que sí se
                        # movió. Si es un fotograma de vídeo, NO se recorta
                        # en absoluto: se ve completo dentro de esa zona
                        # (contener=True), igual que en el layout automático.
                        x, y, w, h = _zona_fallback_contenido(idx_f)
                        foto_zona(c, f["ruta"], x, y, w, h, contener=bool(f.get("es_frame_video")))
                ...
                # Textos sueltos adicionales (el "+T" de una página de
                # contenido, además del pie de foto de siempre) - cada uno
                # lleva su propia posición/estilo guardados en 'info',
                # igual que el título/subtítulo de un capítulo.
                for t in textos_libres:
                    if t.get("texto"):
                        _dibujar_texto_editor(c, AW, AH, canvas_w, canvas_h, t.get("info") or {}, t["texto"])
                if do_wm:
                    wm(c, AW * mm, AH * mm)
                ...
                # Un fotograma de vídeo (es_frame_video) no es una foto de
                # verdad que el cliente eligió con cuidado - es solo la
                # imagen de referencia del QR. NO se le recorta nada: se ve
                # la escena COMPLETA dentro de su hueco (contener=True),
                # en vez de rellenar el hueco recortando un lado como se
                # hace con las fotos reales (antes esto solo desactivaba
                # la detección de caras pero seguía recortando para
                # encajar en la proporción del hueco).
                recortes = []
                contenciones = []
                for f in fotos_validas:
                    if f.get("es_frame_video"):
                        recortes.append(None)
                        contenciones.append(True)
                    else:
                        recortes.append(f.get("recorte_frac"))
                        contenciones.append(False)
                ...
                if layout == "1":
                    ...
                    # Diseños nuevos (3D/3T/3B, 4I/4D, 6, 8, 12) - todos
                    # pasan por la función genérica, que ya sabe calcular
                    # sus zonas con calcular_zonas_layout_mm.
                    layout_generico(c, AW, AH, layout, **kwargs)
...
def asignar_frames_video_a_capitulos(diseño, fotos_dict):
    """
    Asigna a su capítulo correspondiente cualquier foto o fotograma de
    vídeo que se haya quedado SIN capítulo asignado - tanto los
    fotogramas de vídeo (es_frame_video=True, que se crean DESPUÉS de que
    la IA decidiera los capítulos, así que nunca podían estar en su
    lista) como cualquier foto normal que la IA, por lo que sea, se haya
    dejado fuera al organizar el libro (no debería pasar casi nunca, pero
    si pasa, tiene que caer en su capítulo por fecha, no perderse en el
    grupo de "sobrantes" al final del libro sin relación con su momento).
    ...
    EXCEPCIÓN IMPORTANTE (solo aplica a fotogramas de vídeo): si la fecha
    del vídeo no es de fiar (fuente_fecha == "modificacion", es decir, no
    venía ni de EXIF ni del nombre del archivo) - el caso típico de un
    vídeo que pasó por "Reducir / Unir vídeo": ese proceso descarga un
    archivo NUEVO, que pierde la fecha real de grabación y se queda con la
    fecha de cuando se volvió a subir (normalmente HOY, la más reciente de
    todo el álbum). Comparar por fecha en ese caso es peor que inútil:
    casi siempre cae en el ÚLTIMO capítulo, sea o no el que le corresponde
    de verdad. Por eso aquí la fecha se ignora del todo y se fuerza la
    comparación por imagen (la miniatura del fotograma) contra TODOS los
    capítulos, sin excepción. Las fotos normales siempre tienen una fecha
    de fiar (EXIF o nombre de archivo), así que a ellas esto no les afecta.
    ...
    ser el mismo día pero visualmente muy distintos), se le manda la foto/
    fotograma + una miniatura de cada capítulo candidato a Claude, para
    que decida por imagen igual que hace con el resto del libro (reconoce
    lugares, ropa, etc.) - no solo por fecha.

    Sin esto, cualquier foto o vídeo sin capítulo claro acabaría siempre
    en el grupo de "sobrantes" al final del libro, desconectado de su
    momento real y de las demás fotos de ese día. Con esto, la página de
    su capítulo correcto simplemente ajusta (3 fotos -> 4) en vez de crear
    una página aparte al final - y ninguna foto ni vídeo del cliente se
    queda fuera del libro.

    No hace nada en modo "sin capítulos" (diseño["capitulos"] vacío) - ahí
    no hay capítulos a los que asignar nada, así que no aplica.
    """
    ...
    # diferencia entre si respecto a la foto/vídeo, no está claro por fecha sola
    ...
    for nombre, foto in fotos_dict.items():
        if any(nombre in cap.get("fotos", []) for cap in capitulos):
            continue  # ya asignada por la IA - nada que hacer aquí

        es_frame = foto.get("es_frame_video", False)
        etiqueta = "Fotograma de vídeo" if es_frame else "Foto"
        # La excepción de "fecha no fiable" solo tiene sentido para
        # fotogramas de vídeo re-procesados - una foto normal siempre trae
        # una fecha real (EXIF o nombre de archivo).
        fecha_no_fiable = es_frame and foto.get("fuente_fecha") == "modificacion"
        ...
        if fecha_no_fiable:
            ...
            if idx_elegido is not None:
                ...
                log(f"{etiqueta} '{nombre}' (fecha no fiable, viene de un vídeo "
        ...
        # Distancia (en segundos) de la foto/vídeo a cada capítulo con fecha
        distancias = []
        ...
        log(f"{etiqueta} '{nombre}' asignado al capítulo "
            ...
            if nombre_v in qr_urls:
                ...
                if mejor_foto and mejor_diff <= 120:
                    ...
                    if frame:
                        ...
                        # No hay foto cercana en el tiempo (por eso se
                        # intentó sacar un fotograma) Y ADEMÁS falló la
                        # extracción (vídeo corrupto, formato raro...). Sin
                        # este plan B, el QR de este vídeo no se colocaba
                        # en NINGÚN sitio del libro y el vídeo desaparecía
                        # sin más - mejor pegarlo a la foto más cercana que
                        # haya, aunque esté lejos en el tiempo, que perder
                        # el vídeo del cliente por completo.
                        qr_map[mejor_foto["nombre"]] = qr_urls[nombre_v]
                        log(f"No se pudo sacar fotograma de '{nombre_v}' y no había foto "
                            f"cercana - se enlaza igualmente a '{mejor_foto['nombre']}' "
                            f"para no perder el vídeo", "!")
                    else:
                        log(f"No se pudo colocar el vídeo '{nombre_v}' en ningún sitio del "
                            f"libro (sin fotograma y sin ninguna foto en todo el álbum) - "
                            f"esto no debería pasar nunca", "!")
...
def generar_pdf_completo(diseño, fotos, videos_rutas, qr_urls, portada_elegida,
    ...
    # -- RED DE SEGURIDAD FINAL, PARA LAS DOS RUTAS POR IGUAL --
    # OJO, este era el agujero real detrás de "el libro salió con más o
    # menos páginas de las contratadas": _completar_paginas_contenido_
    # hasta_objetivo / _recortar_..._hasta_objetivo / _evitar_fotograma_
    # solo SOLO se aplicaban dentro de calcular_estructura_libro() - pero
    # esa función NO se llama cuando hay estructura_editada, que es el
    # camino que se usa SIEMPRE que el pedido pasa por el editor (o sea,
    # prácticamente todos los pedidos reales). Un libro editado por el
    # cliente podía saltarse por completo la verificación del número de
    # páginas, o volver a dejar el fotograma solo si el cliente tocó algo
    # en esa página. Se aplican aquí, al final, después del if/else, para
    # las dos rutas por igual - si el número de páginas ya es el correcto
    # y el fotograma ya está bien colocado (el caso normal, sin tocar
    # nada raro), esto es un no-op; solo corrige si de verdad hace falta.
    #
    # OJO 2: "paginas_objetivo" es el TOTAL de páginas interiores (título
    # de capítulo incluido), no solo las de "contenido" - _completar_.../
    # _recortar_... solo tocan páginas "tipo=contenido", así que hay que
    # descontarles el número de páginas de título ya presentes antes de
    # pasárselo, igual que se hace dentro de calcular_estructura_libro().
    n_titulos_capitulo = sum(1 for p in paginas if p.get("tipo") == "titulo_capitulo")
    objetivo_contenido = max(1, paginas_objetivo - n_titulos_capitulo)
    paginas = _completar_paginas_contenido_hasta_objetivo(paginas, objetivo_contenido, datos["qr_map"], datos["AW"], datos["AH"])
    paginas = _recortar_paginas_contenido_hasta_objetivo(paginas, objetivo_contenido, datos["qr_map"], datos["AW"], datos["AH"])
    _evitar_fotograma_solo(paginas, datos["AW"], datos["AH"])
    # _evitar_fotograma_solo puede volver a cambiar el número de páginas de
    # contenido (si el fotograma estaba solo en su propia página, esa
    # página se ELIMINA al fusionarlo en otra) - se repite la pareja de
    # funciones una vez más para corregirlo de verdad, no solo avisar.
    n_titulos_capitulo = sum(1 for p in paginas if p.get("tipo") == "titulo_capitulo")
    objetivo_contenido = max(1, paginas_objetivo - n_titulos_capitulo)
    paginas = _completar_paginas_contenido_hasta_objetivo(paginas, objetivo_contenido, datos["qr_map"], datos["AW"], datos["AH"])
    paginas = _recortar_paginas_contenido_hasta_objetivo(paginas, objetivo_contenido, datos["qr_map"], datos["AW"], datos["AH"])
