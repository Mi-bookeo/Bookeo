...
def dibujar_foto_editor_libre(c, ruta, foto_info, canvas_w, canvas_h, aw_pt, ah_pt, clip_a_pagina=True):
    ...
    try:
        ...
        # AVISO TEMPORAL para diagnosticar lo de "portada ampliada" - se
        # puede quitar en cuanto quede claro qué está pasando.
        log(f"[DEBUG PORTADA] archivo={os.path.basename(ruta)} "
            f"iw_true={iw_true} ih_true={ih_true} iw_real={iw_real} ih_real={ih_real} "
            f"foto_ref_w={foto_ref_w} foto_ref_h={foto_ref_h} "
            f"escala_x={escala_x:.3f} escala_y={escala_y:.3f} "
            f"crop_x={crop_x} crop_y={crop_y} crop_w={crop_w} crop_h={crop_h}", "🔍")
