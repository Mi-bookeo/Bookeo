...
def calcular_estructura_libro(diseño, fotos_dict, qr_map, portada_elegida, caso_reparto="A", paginas_objetivo=30, AW=200, AH=200):
    ...
    paginas = _recortar_paginas_contenido_hasta_objetivo(paginas, paginas_objetivo, qr_map, AW, AH)
    ...
    # contenido, blanco) - ahora el editor enseña ese mismo número, sin
    # una página fantasma de más que nunca llegaba a imprimirse.
    ...
    # todos, así que sin este paso el recuento de 30 que ya se había
    # fijado arriba podía descuadrarse otra vez sin que nadie lo
    # detectara. El número de páginas de contenido es un compromiso
    # comercial con el cliente (y con lo que se paga a Gelato por
    # página) - tiene que quedar en EXACTAMENTE paginas_objetivo pase lo
    # que pase antes, así que se repite la misma pareja de funciones
    # (completar si faltan / recortar si sobran) al final del todo, no
    # solo antes de tocar el fotograma. _completar_.../_recortar_... solo
    # tocan páginas "tipo=contenido" - no afectan a la guarda en blanco ni
    # a la contraportada ya añadidas arriba.
    paginas = _completar_paginas_contenido_hasta_objetivo(paginas, paginas_objetivo, qr_map, AW, AH)
    paginas = _recortar_paginas_contenido_hasta_objetivo(paginas, paginas_objetivo, qr_map, AW, AH)
    total_contenido_final = sum(1 for p in paginas if p.get("tipo") == "contenido")
    if total_contenido_final != paginas_objetivo:
        ...
        log(f"⚠️ AVISO IMPORTANTE: el libro quedó con {total_contenido_final} páginas de "
            f"contenido en vez de las {paginas_objetivo} contratadas - revisar el pedido a "
            f"mano antes de mandarlo a Gelato.", "!")
...
def generar_pdf_completo(diseño, fotos, videos_rutas, qr_urls, portada_elegida,
    ...
    paginas = _completar_paginas_contenido_hasta_objetivo(paginas, paginas_objetivo, datos["qr_map"], datos["AW"], datos["AH"])
    paginas = _recortar_paginas_contenido_hasta_objetivo(paginas, paginas_objetivo, datos["qr_map"], datos["AW"], datos["AH"])
