...
def layout_generico(c, AW, AH, layout, fotos_rutas, qr_idx, qr_url, pie, do_wm, qr_pos=None, fondo=None, recortes=None):
    """
    ...
    for i, r in enumerate(fotos_rutas):
        ...
        foto_zona(c, r, x, y, w, h, recorte_frac=rf)
        if qr_idx == i:
...
def layout_titulo_capitulo(c, AW, AH, titulo, subtitulo, fotos_capitulo, variante, do_wm, color_fondo=None):
    ...
    OJO: antes esta función llamaba a foto_zona() sin pasarle ningún
    recorte, así que SIEMPRE caía en su cálculo por defecto (con
    detección de caras) - a diferencia de las páginas de contenido
    normales, que sí calculan y pasan su propio recorte de antemano. Por
    eso un fotograma de vídeo usado como foto de un capítulo seguía
    saliendo recortado "a lo cara" aunque ya estuviera arreglado en el
    resto del libro. Ahora se calcula aquí también, respetando
    es_frame_video igual que en todos los demás sitios.
    """
    ...
    def _recorte(f, w_mm, h_mm):
        es_frame = isinstance(f, dict) and f.get("es_frame_video", False)
        return calcular_ventana_recorte_frac(_ruta(f), w_mm, h_mm, sin_deteccion_caras=es_frame)
    ...
    if n == 0:
        ...
        f = fotos_capitulo[0]
        foto_zona(c, _ruta(f), MG, foto_y, w_z, h_z, recorte_frac=_recorte(f, w_z, h_z))
    elif variante % 2 == 0 or n == 2:
        fw = (AW - MG * 2 - GAP) / 2
        for j, f in enumerate(fotos_capitulo[:2]):
            foto_zona(c, _ruta(f), MG + j * (fw + GAP), foto_y, fw, foto_h, recorte_frac=_recorte(f, fw, foto_h))
    else:
        w_z, h_z = AW - MG * 2, foto_h
        f = fotos_capitulo[0]
        foto_zona(c, _ruta(f), MG, foto_y, w_z, h_z, recorte_frac=_recorte(f, w_z, h_z))
    ...
    while intentos < paginas_objetivo * 4:
        ...
        for a in range(len(idxs)):
            ...
            for b in range(a + 1, len(idxs)):
                ...
                if len(fotos1) + len(fotos2) > MAX_FOTOS_POR_PAGINA:
                    continue
        ...
        if not mejor_pareja:
            log(f"No se pudo recortar más páginas de contenido ({len(idxs)}/{paginas_objetivo}) "
                f"- todas al máximo de fotos por página", "!")
            break
    ...
    while i < len(paginas):
        ...
        try:
            ...
            if idx_frame < len(zonas_nuevas):
                ...
                # sin_deteccion_caras=True: un fotograma de vídeo no debe
                # recortarse "a lo cara" como una foto real - se quiere
                # ver la escena entera del vídeo, no un primer plano.
                frame_foto["recorte_frac"] = calcular_ventana_recorte_frac(
                    frame_foto["ruta"], w_mm, h_mm, sin_deteccion_caras=True
                )
        except Exception as e:
...
def _pagina_contenido_desde_calculo(pg, AW, AH):
    ...
    for i, foto in enumerate(fotos_out):
        if i < len(zonas):
            ...
            # toque a mano) - el intento de arreglo anterior vivía en el
            # dibujado del PDF, un sitio que casi nunca se llegaba a usar
            # porque el recorte YA llegaba fijado desde aquí. Con
            # sin_deteccion_caras=True para los fotogramas de vídeo, se
            # ve la escena entera del vídeo en vez de hacer zoom sobre
            # una cara detectada, tanto en el editor como en el PDF final.
            foto["recorte_frac"] = calcular_ventana_recorte_frac(
                foto["ruta"], w_mm, h_mm, sin_deteccion_caras=foto.get("es_frame_video", False)
            )
    ...
    for pagina in paginas:
        ...
        if tipo == "blanco":
            ...
            if modo_libre:
                ...
                # Los fotogramas de vídeo (es_frame_video) casi nunca
                # traían "recorte_frac" precalculado en este camino (solo
                # se calculaba si la página se había reorganizado por una
                # fusión) - sin él, foto_zona()/recortar_con_caras() caían
                # en la detección de caras por defecto, igual que una foto
                # de verdad, así que un fotograma con una cara detectada
                # salía recortado "a lo cara" en vez de mostrar la escena
                # completa del vídeo. Aquí se calcula de antemano, con el
                # tamaño real del hueco que le toca en ESTE layout
                # concreto, forzando el recorte centrado neutro.
                try:
                    zonas_para_recorte = calcular_zonas_layout_mm(
                        layout, len(fotos_validas), AW, AH,
                        pie_h=8 if pagina.get("texto") else 0,
                    )
                except Exception:
                    zonas_para_recorte = []
                recortes = []
                for idx_f, f in enumerate(fotos_validas):
                    rf = f.get("recorte_frac")
                    if rf is None and f.get("es_frame_video") and idx_f < len(zonas_para_recorte):
                        _, _, w_mm_zona, h_mm_zona = zonas_para_recorte[idx_f]
                        rf = calcular_ventana_recorte_frac(
                            f["ruta"], w_mm_zona, h_mm_zona, sin_deteccion_caras=True
                        )
                    recortes.append(rf)
